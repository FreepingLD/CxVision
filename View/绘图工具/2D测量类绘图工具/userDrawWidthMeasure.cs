...
{
    [Serializable]
    public class userDrawWidthMeasure : DrawingBaseMeasure
    {
        private Rect2 rect2;
        ...
        public userDrawWidthMeasure(HWindowControl hWindowControl) : base(hWindowControl)
        {
            
        }
        public userDrawWidthMeasure(HWindowControl hWindowControl, userWcsRectangle2 wcsRect2, userWcsCoordSystem wcsCoordSystem) : base(hWindowControl)
        {
        ...
        public userDrawWidthMeasure(HWindowControl hWindowControl, userPixRectangle2 pixRect2, userPixCoordSystem pixCoordSystem) : base(hWindowControl)
        {
        ...
        public userDrawWidthMeasure(HWindowControl hWindowControl, userWcsRectangle2 wcsRect2) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (wcsRect2 != null && wcsRect2.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        public userDrawWidthMeasure(HWindowControl hWindowControl, userPixRectangle2 pixRect2) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (pixRect2 != null && pixRect2.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        protected override void hWindowControl_HMouseMove(object sender, HMouseEventArgs e)
        {
            ...
            switch (selectedNode)
            {
                ...
                case PosSizableRect.Arrow:
                    if (e.Button == MouseButtons.Left)
                    {
                        this.rect2.diffRadius += HMisc.DistancePp(e.Y, e.X, this.rect2.row, this.rect2.col) - HMisc.DistancePp(this.oldY, this.oldX, this.rect2.row, this.rect2.col);
                    }
                    else
                    {
                        HTuple ATan;
        ...
        {
            rows = new double[(num) * 2];
            cols = new double[(num) * 2];
            phis = new double[(num) * 2];
            ...
            double dist2Mea = HMisc.DistancePp(rightDownRow, rightDownCol, leftDownRow, leftDownCol) / (num - 1);
            //double dist2Mea = HMisc.DistancePp(leftDownRow, leftDownCol, leftUpRow, leftUpCol) / (num - 1);
            //////////////////////////////////////////////////////
            for (int i = 0; i < num; i++)
            {
            ...
            for (int i = 0; i < num; i++)
            {
                GetPointLine(rightDownRow, rightDownCol, leftDownRow, leftDownCol, dist2Mea * i, out row, out col);
                rows[i + num * 1] = row;
                cols[i + num * 1] = col;
                phis[i + num * 1] = Math.Atan2((rightDownRow - leftDownRow) * -1, rightDownCol - leftDownCol) - Math.PI * 0.5;
            }
            ////////////////////////
            //for (int i = 0; i < num; i++)
            //{
            //    GetPointLine(leftDownRow, leftDownCol, leftUpRow, leftUpCol, dist2Mea * i, out row, out col);
            //    rows[i + num * 1] = row;
            //    cols[i + num * 1] = col;
            //    phis[i + num * 1] = Math.Atan2((leftDownRow - leftUpRow) * -1, leftDownCol - leftUpCol) - Math.PI * 0.5;
            //}
        }
