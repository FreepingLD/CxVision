...
{
    [Serializable]
    public class userDrawRect2Measure : DrawingBaseMeasure
    {
        private Rect2 rect2;
        ...
        public userDrawRect2Measure(HWindowControl hWindowControl) : base(hWindowControl)
        {
            
        }
        public userDrawRect2Measure(HWindowControl hWindowControl, userWcsRectangle2 wcsRect2, userWcsCoordSystem wcsCoordSystem) : base(hWindowControl)
        {
        ...
        public userDrawRect2Measure(HWindowControl hWindowControl, userPixRectangle2 pixRect2, userPixCoordSystem pixCoordSystem) : base(hWindowControl)
        {
        ...
        public userDrawRect2Measure(HWindowControl hWindowControl, userWcsRectangle2 wcsRect2) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (wcsRect2 != null && wcsRect2.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        public userDrawRect2Measure(HWindowControl hWindowControl, userPixRectangle2 pixRect2) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (pixRect2 != null && pixRect2.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        protected override void hWindowControl_HMouseMove(object sender, HMouseEventArgs e)
        {
            ...
            switch (selectedNode)
            {
                ...
                case PosSizableRect.Arrow:
                    if (e.Button == MouseButtons.Left)
                    {
                        //this.rect2.diffRadius += HMisc.DistancePp(e.Y, e.X, this.oldY, this.oldX)*((e.Y - this.oldY)/Math.Abs(e.Y - this.oldY)) * ((e.X - this.oldX) / Math.Abs(e.X - this.oldX));
        ...
        private PosSizableRect GetNodeSelectable(double x, double y)
        {
            ...
            if (this.rect2.GetHXLD(this.rect2.diffRadius * 1.1).TestXldPoint(y, x) > 0)
