...
{
    [Serializable]
    public class userDrawManualCircleSectorMeasure : DrawingBaseMeasure
    {
        private int state = 0;
        ...
        public userDrawManualCircleSectorMeasure(HWindowControl hWindowControl) : base(hWindowControl)
        {

        }
        public userDrawManualCircleSectorMeasure(HWindowControl hWindowControl, userWcsCircleSector wcsCircleSector, userWcsCoordSystem wcsCoordSystem) : base(hWindowControl)
        {
        ...
        public userDrawManualCircleSectorMeasure(HWindowControl hWindowControl, userPixCircleSector pixCircleSector, userPixCoordSystem pixCoordSystem) : base(hWindowControl)
        {
        ...
        public userDrawManualCircleSectorMeasure(HWindowControl hWindowControl, userWcsCircleSector wcsCircleSector) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (wcsCircleSector != null && wcsCircleSector.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        public userDrawManualCircleSectorMeasure(HWindowControl hWindowControl, userPixCircleSector pixCircleSector) : base(hWindowControl)
        {
            this.pixCoordSystem = new userPixCoordSystem();
            if (pixCircleSector != null && pixCircleSector.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
            ...
            if (param != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
                switch (param?.GetType().Name)
                {
                    ...
                    case nameof(String):
                        ...
                        {
                            case "圆心X":
                                double.TryParse(value[1].ToString(), out result);
                                this.CameraParam.WorldPointsToImagePlane(result, 0, 0, 0, 0, 0, out row, out col);
                                this.currentDrawCircle.col = col;
                                break;
                            case "圆心Y":
                                double.TryParse(value[1].ToString(), out result);
                                this.CameraParam.WorldPointsToImagePlane(0, result, 0, 0, 0, 0, out row, out col);
                                this.currentDrawCircle.row = row;
                                break;
                            case "半径":
                                double.TryParse(value[1].ToString(),out result);
                                radius = this.CameraParam.TransWcsLengthToPixLength(result);
                                this.currentDrawCircle.radius = radius;
                                break;
                            case "起始角":
                                double.TryParse(value[1].ToString(), out result);
                                this.currentDrawCircle.start_phi = result * Math.PI / 180;
                                break;
                            case "终止角":
                                double.TryParse(value[1].ToString(), out result);
                                this.currentDrawCircle.end_phi  = result * Math.PI / 180;
                                break;
                            case "刷新":
                                this.currentDrawCircle.startPointRow = this.currentDrawCircle.row - currentDrawCircle.radius * Math.Sin(this.currentDrawCircle.start_phi);
                                this.currentDrawCircle.startPointCol = this.currentDrawCircle.col + currentDrawCircle.radius * Math.Cos(this.currentDrawCircle.start_phi);
                                this.currentDrawCircle.middlePointRow = this.currentDrawCircle.row - currentDrawCircle.radius * Math.Sin((this.currentDrawCircle.start_phi + this.currentDrawCircle.end_phi)*0.5);
                                this.currentDrawCircle.middlePointCol = this.currentDrawCircle.col + currentDrawCircle.radius * Math.Cos((this.currentDrawCircle.start_phi + this.currentDrawCircle.end_phi) * 0.5);
                                this.currentDrawCircle.endPointRow = this.currentDrawCircle.row - currentDrawCircle.radius * Math.Sin(this.currentDrawCircle.end_phi);
                                this.currentDrawCircle.endPointCol = this.currentDrawCircle.col + currentDrawCircle.radius * Math.Cos(this.currentDrawCircle.end_phi);
                                this.FitCircleParam();
        ...
        public override object GetParam(string paramName)
        {
            object value = 0;
            double x=0, y=0,z=0;
            switch (paramName)
            {
                case "圆心X":
                    this.CameraParam.ImagePointsToWorldPlane(this.currentDrawCircle.row, this.currentDrawCircle.col, 0, 0, 0,  out x, out y,out z);
                    value = x;
                    break;
                case "圆心Y":
                    this.CameraParam.ImagePointsToWorldPlane(this.currentDrawCircle.row, this.currentDrawCircle.col, 0, 0, 0, out x, out y, out z);
                    ...
                    break;
                case "半径":
                    value  = this.CameraParam.TransPixLengthToWcsLength(this.currentDrawCircle.radius);
                    break;
                case "起始角":
                    value = this.currentDrawCircle.start_phi * 180 / Math.PI;
                    break;
                case "终止角":
                    value = this.currentDrawCircle.end_phi * 180 / Math.PI;
                    break;
            }
            return value;   
        }
                ...
                foreach (PosSizableRect pos in Enum.GetValues(typeof(PosSizableRect)))
                {
                    ...
                    switch (pos)
                    {
                        case PosSizableRect.StartPoint:
                            //
                            if (this.isDispalyAttachDrawingProperty)
                                this.AttachDrawingPropertyData.Add(new ViewData(GetDrawingObject(pos), "yellow"));
                            break;
                        default:
                            if (this.isDispalyAttachDrawingProperty)
                                this.AttachDrawingPropertyData.Add(new ViewData(GetDrawingObject(pos), "red")); //GetDrawingObject(pos)
                            break;
                    }
                }
            }
