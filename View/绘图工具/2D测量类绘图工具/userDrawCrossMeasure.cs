...
{
    [Serializable]
    public class userDrawCrossMeasure : DrawingBaseMeasure
    {
        private Cross lineStruct;
        ...
        public userDrawCrossMeasure(HWindowControl hWindowControl) : base(hWindowControl)
        {

        }
        public userDrawCrossMeasure(HWindowControl hWindowControl, userWcsLine wcsLine, userWcsCoordSystem wcsCoordSystem) : base(hWindowControl)
        {
            ...
            if (wcsLine != null && wcsLine.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
                ...
                this.lineStruct = new Cross(pixLine.Row1, pixLine.Col1, pixLine.Row2, pixLine.Col2, pixLine.DiffRadius);
                //this.wcsLine = wcsLine;
            }
            else
            {
                ...
                this.lineStruct = new Cross(row1 + (row2 - row1) * 0.1, col1 + (col2 - col1) * 0.1, row1 + (row2 - row1) * 0.3, col1 + (col2 - col1) * 0.1, 20);
        ...
        public userDrawCrossMeasure(HWindowControl hWindowControl, userPixLine pixLine, userPixCoordSystem pixCoordSystem) : base(hWindowControl)
        {
            ...
            if (pixLine != null && pixLine.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
                ...
                this.lineStruct = new Cross(pixLine2.Row1, pixLine2.Col1, pixLine2.Row2, pixLine2.Col2, pixLine2.DiffRadius);
            }
            else
            {
                int row1, col1, row2, col2;
        ...
        public userDrawCrossMeasure(HWindowControl hWindowControl, userWcsLine wcsLine) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (wcsLine != null && wcsLine.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        public userDrawCrossMeasure(HWindowControl hWindowControl, userPixLine pixLine) : base(hWindowControl)
        {
            this.wcsCoordSystem = new userWcsCoordSystem();
            if (pixLine != null && pixLine.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
        ...
        protected override void hWindowControl_HMouseMove(object sender, HMouseEventArgs e)
        {
            ...
            switch (selectedNode)
            {
                case PosSizableRect.StartRect:
                    ...
                    this.lineStruct.startPointCol2 += e.Y - oldY;
                    this.lineStruct.endPointCol2 -= e.Y - oldY;
                    break;

                case PosSizableRect.StartRect2:
                    this.lineStruct.startPointCol2 += e.X - oldX;
                    this.lineStruct.endPointCol2 -= e.X - oldX;

                    this.lineStruct.startPointRow += e.X - oldX;
                    this.lineStruct.endPointRow -= e.X - oldX;
                    break;
                ...
                case PosSizableRect.MiddleRect:
                    ...
                    ////////////////////////////////////
                    this.lineStruct.startPointCol2 += e.X - oldX;
                    this.lineStruct.startPointRow2 += e.Y - oldY;
                    this.lineStruct.endPointCol2 += e.X - oldX;
                    this.lineStruct.endPointRow2 += e.Y - oldY;
                    break;

                case PosSizableRect.Arrow:
                case PosSizableRect.Arrow2:
        ...
        private HXLDCont CreateLine()
        {
            ...
            rect2.GenContourPolygonXld(new HTuple(this.lineStruct.startPointRow2, this.lineStruct.endPointRow2), new HTuple(this.lineStruct.startPointCol2, this.lineStruct.endPointCol2));
            return rect.ConcatObj(rect2);
        }
        private HXLDCont GetSizableRect(PosSizableRect p)
        {
            switch (p)
            {
                ...
                case PosSizableRect.StartRect2:
                    return CreateRectSizableNode(this.lineStruct.startPointCol2, this.lineStruct.startPointRow2);
                case PosSizableRect.EndRect:
                    return CreateRectSizableNode(this.lineStruct.endPointCol, this.lineStruct.endPointRow);
                case PosSizableRect.EndRect2:
                    return CreateRectSizableNode(this.lineStruct.endPointCol2, this.lineStruct.endPointRow2);
                case PosSizableRect.MiddleRect:
        ...
        private PosSizableRect GetNodeSelectable(double x, double y)
        {
            ...
            double minDist, maxDist, minDist2, maxDist2;
            ...
            HMisc.DistancePs(y, x, this.lineStruct.startPointRow2, this.lineStruct.startPointCol2, this.lineStruct.endPointRow2, this.lineStruct.endPointCol2, out minDist2, out maxDist2);
            ...
            ////////////////////////////////////////
            double dist3 = Math.Sqrt((y - this.lineStruct.startPointRow2) * (y - this.lineStruct.startPointRow2) + (x - this.lineStruct.startPointCol2) * (x - this.lineStruct.startPointCol2));
            double dist4 = Math.Sqrt((y - this.lineStruct.endPointRow2) * (y - this.lineStruct.endPointRow2) + (x - this.lineStruct.endPointCol2) * (x - this.lineStruct.endPointCol2));
            //////////////////////////////////
            if (midDist < this.nodeSizeRect * 2) // 表示移动
            ...
            if (dist3 < this.nodeSizeRect)
                selectNode = PosSizableRect.StartRect2;
            //////////////////////////////////////////////////第三点
            if (dist4 < this.nodeSizeRect)
                selectNode = PosSizableRect.EndRect2;

            if (Math.Abs(this.lineStruct.diffRadius) * 0.8 < minDist && minDist < Math.Abs(this.lineStruct.diffRadius) * 1.2)
                selectNode = PosSizableRect.Arrow;
            if (Math.Abs(this.lineStruct.diffRadius) * 0.8 < minDist2 && minDist2 < Math.Abs(this.lineStruct.diffRadius) * 1.2)
                selectNode = PosSizableRect.Arrow2;
        ...
        private Cursor GetCursor(PosSizableRect p)
        {
            switch (p)
            {
                ...
                case PosSizableRect.StartRect2:  //MovePos
                case PosSizableRect.EndRect2:  //MovePos
        ...
        private struct Cross
        {
            ...
            public double startPointRow2;
            public double startPointCol2;
            public double endPointRow2;
            public double endPointCol2;

            public double diffRadius;
            public double normalPhi;

            public Cross(double row1, double col1, double row2, double col2, double diffRadius)
            {
                ...
                hHomMat2D.VectorAngleToRigid((this.startPointRow + this.endPointRow) * 0.5, (this.startPointCol + this.endPointCol) * 0.5, Math.Atan2(this.endPointRow - this.startPointRow, this.endPointCol - this.startPointCol),
                                             (this.startPointRow + this.endPointRow) * 0.5, (this.startPointCol + this.endPointCol) * 0.5, Math.Atan2(this.endPointRow - this.startPointRow, this.endPointCol - this.startPointCol) + Math.PI * 0.5);
                Qx = hHomMat2D.AffineTransPoint2d(new HTuple(this.startPointRow, this.endPointRow), new HTuple(this.startPointCol, this.endPointCol), out Qy);
                this.startPointRow2 = Qx[0].D;
                this.startPointCol2 = Qy[0].D;
                this.endPointRow2 = Qx[1].D;
                this.endPointCol2 = Qy[1].D;
                ///////////////////////////
        ...
        public override void AttachDrawingObjectToWindow()
        {
            ...
            if (this.lineStruct.startPointRow == 0 && this.lineStruct.endPointRow == 0)
                this.lineStruct = new Cross(row1 + 200, column1 + 200, row1 + 400, column1 + 200, 20);
        ...
        private void GetNormalCoordPoint(double[] rows, double[] cols, double normalPhi, double normalLength, out double[] Row1, out double[] Col1, out double[] Row2, out double[] Col2)
        {
            ...
            for (int i = 0; i < rows.Length; i++)
            {
                ...
                {
                    Row1[i] = rows[i] + normalLength * Math.Sin(normalPhi + Math.PI * 0.5); // 因为像素坐标系与世界坐标系Y轴相反，所以这里Y也要用相反的操作符
                    Col1[i] = cols[i] - normalLength * Math.Cos(normalPhi + Math.PI * 0.5);
                    Row2[i] = rows[i] - normalLength * Math.Sin(normalPhi + Math.PI * 0.5);
                    Col2[i] = cols[i] + normalLength * Math.Cos(normalPhi + Math.PI * 0.5);
                }
            }
        }
        private void GetArrowPoint(Cross pixLine, int Num, out double[] Row1, out double[] Col1, out double[] Row2, out double[] Col2)
        {
            ...
            ////////////////////////////////////////////
            double[] rows = new double[Num * 2];
            double[] cols = new double[Num * 2];
            for (int i = 0; i < Num; i++)
            {
            ...
            {
                GetPointLine(pixLine.startPointRow2, pixLine.startPointCol2, pixLine.endPointRow2, pixLine.endPointCol2, (i - Num) * dist1Mea, out rows[i], out cols[i]);
            }
            /////////////////////////////////////////
            ...
            //this.lineStruct.normalPhi2 = Math.Atan2((Row2[5] - Row1[5]) * -1, Col2[5] - Col1[5]);
        }
        ...
        public HXLDCont GetArrowXLD()
        {
            ...
            switch (this.ShowMode)
            {
                ...
                case enShowMode.箭头:
                    ...
                    break;
                case enShowMode.矩形:
                    GetArrowPoint(this.lineStruct, 3, out row1, out col1, out row2, out col2);
