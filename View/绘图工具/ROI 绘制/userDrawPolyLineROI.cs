...
{
    [Serializable]
    public class userDrawPolyLineROI : VisualizeView 
    {
        ...
        public userDrawPolyLineROI(HWindowControl hWindowControl, bool appendContextMenuStrip) : base(hWindowControl, appendContextMenuStrip)
        {

        }
        public userDrawPolyLineROI(HWindowControl hWindowControl, userPixLine pixLine) : base(hWindowControl)
        {
        ...
        protected override void hWindowControl_HMouseMove(object sender, HMouseEventArgs e)
        {
            ...
            switch (selectedNode)
            {
                ...
                case PosSizableRect.MiddleNode: // 移动直线的中点时，将其转化为节点
                    ...
                    LastIndex = 0;
                    FirstIndex = 0;
                    //if (this.pixPolyLine.ActiveIndex == 0)
                    //{
                    //    FirstIndex = 0;
                    //    LastIndex = this.pixPolyLine.ActiveIndex + 1;
                    //}
                    //else if (this.pixPolyLine.ActiveIndex == this.pixPolyLine.CircleRow.Count - 1)
                    //{
        ...
        {
            userPixPolygon pixPolygon = new userPixPolygon(this.pixPolyLine.RectRow.ToArray(), this.pixPolyLine.RectCol.ToArray(), this.CameraParam);
            pixPolygon = pixPolygon.AffinePixPolygon(this.pixCoordSystem?.GetInvertVariationHomMat2D()); // 变换为之前的
            return pixPolygon;
        }
        public drawPixPolyLine GetDrawPixPolygonParam()
        {
            drawPixPolyLine pixPolygon = new drawPixPolyLine(this.pixPolyLine.RectRow.ToArray(), this.pixPolyLine.RectCol.ToArray());
            pixPolygon = pixPolygon.AffinePixPolygon(this.pixCoordSystem?.GetInvertVariationHomMat2D()); // 变换为之前的
            return pixPolygon;
        }
    }

    public class PixPolyLine
    {
        ...
        public PixPolyLine(double[] rows, double[] cols)
        {
            this.RectRow = new List<double>();
            this.RectCol = new List<double>();
            this.CircleRow = new List<double>();
        ...
        public PixPolyLine(userPixRectangle1 pixRectangle1)
        {
            this.RectRow = new List<double>();
            this.RectCol = new List<double>();
            this.CircleRow = new List<double>();
