    ...
    public class userDrawRect2ROI : VisualizeView
    {
            ...
            if (param != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
                switch (param.GetType().Name)
                {
                    case nameof(userPixRectangle2):
                        ...
                        {
                            userPixRectangle2 pixRect = pixRect2;//.GetPixRectangle2();
                            this.rect2 = new Rect2(pixRect.Row, pixRect.Col, pixRect.Rad, pixRect.Length1, pixRect.Length2, 0);
                        }
                        else
                        {
                            int row1, col1, row2, col2;
                    ...
                    case nameof(drawPixRect2):
                        if (!(param is drawPixRect2)) throw new ArgumentException("给定的数据类型异常，需要传入 drawPixRect2 类型");
                        drawPixRect2 PixRect2 = (drawPixRect2)param;
                        this.rect2 = new Rect2(PixRect2.Row, PixRect2.Col, PixRect2.Rad, PixRect2.Length1, PixRect2.Length2, 0);
                        this.isDrawMode = true;
                        break;
                    case nameof(drawWcsRect2):
                        if (!(param is drawWcsRect2)) throw new ArgumentException("给定的数据类型异常，需要传入 drawWcsRect2 类型");
                        drawWcsRect2 WcsRect2 = (drawWcsRect2)param;
                        if (this.CameraParam != null)
                        {
                            drawPixRect2 pixRect = WcsRect2.GetPixRect2(this.CameraParam);//.GetPixRectangle2();
                            this.rect2 = new Rect2(pixRect.Row, pixRect.Col, pixRect.Rad, pixRect.Length1, pixRect.Length2, 0);
                        }
                        else
                        {
        ...
        public override void DrawWcsRoiShapeOnWindow(enColor color, out WcsROI roi)
        {
            ...
            while (true)
            {
                ...
                if (MouseButtons.Right == this.CurrentButton || MouseButtons.None == this.CurrentButton) break;
                Thread.Sleep(10);
                if (stopwatch.ElapsedMilliseconds > 60 * 2 * 1000) break; //超过两分钟
            }
            stopwatch.Stop();
            this.isDrawMode = false;
            this.isTranslate = true;
        ...
        public drawWcsRect2 GetDrawWcsRect2Param()
        {
            drawPixRect2 PixRectangle2 = new drawPixRect2(this.rect2.row, this.rect2.col, this.rect2.phi, this.rect2.length1, this.rect2.length2);
            drawWcsRect2 wcsRect2 = PixRectangle2.GetWcsRect2(this.CameraParam);
            wcsRect2 = wcsRect2.AffineTransWcsRect2(this.wcsCoordSystem?.GetInvertVariationHomMat2D());
            return wcsRect2;
        }
