...
{
    [Serializable]
    public class userDrawEllipseROI : VisualizeView
    {
        ...
        public userDrawEllipseROI(HWindowControl hWindowControl, bool appendContextMenuStrip) : base(hWindowControl, appendContextMenuStrip)
        {

        }

        public userDrawEllipseROI(HWindowControl hWindowControl, userPixEllipse pixEllipse, userPixCoordSystem pixCoordSystem) : base(hWindowControl)
        {
            if (pixEllipse != null && pixEllipse.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
                ...
                userPixEllipse PixEllipse = pixEllipse.AffineTransPixEllipse(pixCoordSystem.GetVariationHomMat2D());//.GetPixEllipse();
        ...
        public userDrawEllipseROI(HWindowControl hWindowControl, userPixEllipse pixEllipse) : base(hWindowControl)
        {
            if (pixEllipse != null && pixEllipse.CamParams != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
            ...
            if (param != null)// 等于null表示是第一次使用，否则则不是第一次使用
            {
                switch (param.GetType().Name)
                {
                    ...
                    case nameof(drawPixEllipse):
                        if (!(param is drawPixEllipse)) throw new ArgumentException("给定的数据类型异常，需要传入 drawPixEllipse 类型");
                        drawPixEllipse pixEllipse = (drawPixEllipse)param;
                        //this.ellipse = new Ellipse(pixEllipse.Row, pixEllipse.Col, pixEllipse.Rad, pixEllipse.Radius1, pixEllipse.Radius2, 25);
                        //this.ellipse.diffRadius = 25;
                    ...
                    case nameof(drawWcsEllipse):
                        if (!(param is drawWcsEllipse)) throw new ArgumentException("给定的数据类型异常，需要传入 drawWcsEllipse 类型");
                        drawWcsEllipse wcsCircle = (drawWcsEllipse)param;
                        if (this.CameraParam != null)
                        {
                            pixEllipse = wcsCircle.GetPixEllipse(this.CameraParam);
                            this.ellipse = new Ellipse(pixEllipse.Row, pixEllipse.Col, pixEllipse.Rad, pixEllipse.Radius1, pixEllipse.Radius2, 25);
                            this.ellipse.diffRadius = 25;
                        }
                        else
                    ...
                    case nameof(userWcsCoordSystem):
                        if (param != null)
                            this.wcsCoordSystem = ((userWcsCoordSystem)param);//.GetPixCoordSystem();
                        else
        ...
        public override void DrawRoiShapeOnWindow(enColor color, out ROI roi)
        {
            ...
            roi = this.GetDrawPixEllipseParam();
        ...
        public override void DrawPixRoiShapeOnWindow(enColor color, out PixROI roi)
        {
            ...
            this.CurrentButton = MouseButtons.Left;
            this.mIsClick = false;
            this.CameraParam.CaliParam.CamCaliModel = enCamCaliModel.CamParamPose; // 这里必需要设置一下，绘㓡世界ROI，表示是在3D模型上绘制
            this.DrawingGraphicObject();
            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Restart();
            while (true)
        ...
        public override void DrawWcsRoiShapeOnWindow(enColor color, out WcsROI roi)
        {
            this.isTranslate = false;
            this.isDisplayHImageData = true;
            this.isDrawMode = true;
            ...
            this.isDisplayHImageData = true;
            this.CurrentButton = MouseButtons.None;
            roi = this.GetDrawWcsEllipseParam();
            this.DetachDrawingObjectFromWindow();
        }
        public override void DrawPixEllipseOnWindow(enColor color, out userPixEllipse pixEllipse)
        {            ////// 等待结束 //////////
            this.isTranslate = false;
            this.isDisplayHImageData = true;
            this.isDrawMode = true;
            ...
            this.isDisplayHImageData = true;
            this.CurrentButton = MouseButtons.None;
            pixEllipse = this.GetPixEllipseParam();
            this.DetachDrawingObjectFromWindow();
        }
        public override void DrawPixEllipseOnWindow(enColor color, out drawPixEllipse pixEllipse)
        {            ////// 等待结束 //////////
            this.isTranslate = false;
            this.isDisplayHImageData = true;
            this.isDrawMode = true;
            ...
            this.isDisplayHImageData = true;
            this.CurrentButton = MouseButtons.None;
            pixEllipse = this.GetDrawPixEllipseParam();
            this.DetachDrawingObjectFromWindow();
        }
        ...
        public drawPixEllipse GetDrawPixEllipseParam()
        {
            drawPixEllipse pixEllipse = new drawPixEllipse(this.ellipse.row, this.ellipse.col, this.ellipse.phi, this.ellipse.radius1, this.ellipse.radius2);
            pixEllipse = pixEllipse.AffinePixEllipse(this.pixCoordSystem?.GetInvertVariationHomMat2D());
            return pixEllipse;
        }

        public drawWcsEllipse GetDrawWcsEllipseParam()
        {
            drawPixEllipse pixEllipse = new drawPixEllipse(this.ellipse.row, this.ellipse.col, this.ellipse.phi, this.ellipse.radius1, this.ellipse.radius2);
            drawWcsEllipse wcsEllipse = pixEllipse.GetWcsEllipse(this.CameraParam);
            wcsEllipse = wcsEllipse.AffineTransWcsEllipse(this.wcsCoordSystem?.GetInvertVariationHomMat2D());
            return wcsEllipse;
        }

        public override userPixEllipse GetPixEllipseParam()
        {
            userPixEllipse pixEllipse = new userPixEllipse(this.ellipse.row, this.ellipse.col, this.ellipse.phi, this.ellipse.radius1, this.ellipse.radius2, this.CameraParam);
