...
namespace Sensor
{
    public class DalsaLineCam : SensorBase, ISensor
    {
        //private HImage _grabImage;
        //private HImage _grabDarkImage;
        private CLDalsa _camera;
            ...
            try
            {
                ...
                switch (configParam.ConnectType)
                {
                    ...
                    case enUserConnectType.SerialNumber:
                        result = this._camera.Open(configParam.ConnectAddress, configParam.SensorName);
                        ...
                        switch (this.CameraParam.TriggerSource)
                        {
                            ...
                            case enUserTriggerSource.软触发:
                                //this._camera.SetCamPara(true);
                                //this._camera.StartGrab(); // 这里不能开启流
                                break;
                            default:
                                //this._camera.SetCamPara(true);
        ...
        public Dictionary<enDataItem, object> ReadData()
        {
            ...
            list.Add(enDataItem.Image, new ImageDataClass(this._grabImage, this.CameraParam, this._CurrentImageIndex)); //this.imageData  .Clone()
            //if (this._grabDarkImage != null && this._grabDarkImage.IsInitialized())
            list.Add(enDataItem.DarkImage, new ImageDataClass(this._grabDarkImage, this.CameraParam, this._CurrentImageIndex)); //this.imageData .Clone()
            return list;
        }
                        ...
                        case enUserTriggerSource.编码器触发:
                            ...
                            switch (this.CameraParam.AcqMode)
                            {
                                ...
                                case enAcqMode.异步采集: // 异步触发、同步取图、异步停止 
                                    ...
                                    this._camera.ClearData();
                                    result = this._camera.StartGrab();
                                    this.GetImageAsyn();  //  开启异步采图线程
                                    break;
                            }
                            break;
                        case enUserTriggerSource.软触发: // 这里多了一个软触发指令
                            break;
                    }
