...
{
    public class SiemensPlc : MotionBase
    {
        private string sign = "西门子PLC";
        private IReadWriteNet _readWriteNet;
        private SiemensS7Net _TcpNet;
        ...
        public override bool Init(DeviceConnectConfigParam param)
        {
            ...
            try
            {
                ...
                switch (param.ConnectType)
                {
                    ...
                    case enUserConnectType.Network:
                        ...
                        Enum.TryParse<SiemensPLCS>(param.DeviceModel.ToString(), out siemensPLCS);
                        this.adress = param.IpAdress;
                        this._TcpNet?.ConnectClose();
                        this._TcpNet = new SiemensS7Net(siemensPLCS, param.IpAdress);
                        OperateResult connect = this._TcpNet.ConnectServer();
                        if (connect.IsSuccess)
                        {
                            result = true;
                            this._readWriteNet = this._TcpNet;
                            LoggerHelper.Info("西门子PLC" + param.IpAdress + "打开成功");
                        }
                        else
                        {
                            result = false;
                            LoggerHelper.Warn("西门子PLC" + param.IpAdress + "打开失败");
                        }
                        break;
            ...
            {
                result = false;
                LoggerHelper.Error("西门子PLC" + param.IpAdress + "打开报错" + ex.ToString());
            }
        ...
        public override void UnInit()
        {
            ...
            {
                this._TcpNet?.ConnectClose();
                LoggerHelper.Info("西门子PLC" + this.adress + "关闭成功");
            }
            catch (Exception ex)
            {
                LoggerHelper.Error("西门子PLC" + this.adress + "关闭报错" + ex.ToString());
            }
        }
        public override object ReadValue(enDataTypes dataType, string address, int length)
        {
            ...
            if (value != null)
                LoggerHelper.Error("西门子PLC:" + this.adress + "读取数据成功");
            else
                LoggerHelper.Error("西门子PLC:" + this.adress + "读取数据失败");
            //}
            //catch (Exception ex)
            //{
            //    LoggerHelper.Error("西门子PLC:" + this.adress + "读取数据失败", ex);
            //    //return null;
            //}
            return value;
        }
        public override bool WriteValue(enDataTypes dataType, string address, object value)
        {
            ...
            if (content.IsSuccess)
                LoggerHelper.Info("西门子PLC:" + this.adress + "数据写入成功");
            else
                LoggerHelper.Info("西门子PLC:" + this.adress + "数据写入失败");
            //}
            //catch (Exception ex)
            //{
            //    LoggerHelper.Error("西门子PLC:" + this.adress + "写入数据报错", ex);
            //}
            return result;
        }
