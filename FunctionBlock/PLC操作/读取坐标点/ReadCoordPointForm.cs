...
{
    public partial class ReadCoordPointForm : Form
    {
        ...
        public ReadCoordPointForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public ReadCoordPointForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void ReadCoordPointForm_Load(object sender, EventArgs e)
        {
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                //////////////////////////////////////////////////
                this.数据写入dataGridView.DataSource = ((ReadCoordPoint)this._function).ReadDataList;
            }
            catch (Exception ex)
            {
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<ReadDataCommand> readDataList = ((BindingList<ReadDataCommand>)((ReadCoordPoint)this._function).ReadDataList);
