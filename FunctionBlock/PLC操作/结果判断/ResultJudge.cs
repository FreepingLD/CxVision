...
namespace FunctionBlock
{
    ...
    /// 激光取点采集的数据类
    /// </summary>
    public class ResultJudge : BaseFunction, IFunction
    {
        [NonSerialized]
        ...
        public BindingList<ResultJudgeCommand> DataList { get; set; }
        public ResultJudge()
        {
            this.DataList = new BindingList<ResultJudgeCommand>();
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            try
            {
                ...
                this._content = "OK";
                //////////////////////////////////////////////////////
                BindingList<ResultJudgeCommand> sendDataList = ((BindingList<ResultJudgeCommand>)this.DataList);
                foreach (var item in sendDataList)
                {
                    ...
                    object value = MemoryManager.Instance.GetValue(item.DataSource, enFlag.结果);
                    if (value != null)
                    {
                        string jValue = value.ToString();
                        if (jValue == "OK" || jValue == "ok")
                            item.Result = "OK";
                        else
                            item.Result = "NG";
                    }
                ...
                if (this.Result.Succss)
                    LoggerHelper.Info(this.name + "-结果判断:" + this._content + "成功");
                else
                    LoggerHelper.Error(this.name + "-结果判断:" + this._content + "失败");
            }
            catch (Exception ex)
            {
                this.Result.Succss = false;
                LoggerHelper.Error(this.name + "结果判断：" + "报错" + ex);
            }
            // 更改UI字体　
