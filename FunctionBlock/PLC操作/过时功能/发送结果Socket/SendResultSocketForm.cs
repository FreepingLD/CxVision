...
{
    public partial class SendResultSocketForm : Form
    {
        ...
        public SendResultSocketForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public SendResultSocketForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void SendResultSocketForm_Load(object sender, EventArgs e)
        {
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                //////////////////////////////////////////////////
                this.数据写入dataGridView.DataSource = ((SendResultSocket)this._function).DataList;
            }
            catch (Exception ee)
            {
        ...
        private void SendResultSocketForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (cts != null)
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<WriteCommunicateCommand> WriteDataList = ((BindingList<WriteCommunicateCommand>)((SendResultSocket)this._function).DataList);
        ...
        private void 全部启用checkBox_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (!this.全部启用checkBox.Checked) return;
                this.全部禁用checkBox.Checked = false;
                BindingList<WriteCommunicateCommand> WriteDataList = ((BindingList<WriteCommunicateCommand>)((SendResultSocket)this._function).DataList);
                foreach (var item in WriteDataList)
                {
                    if (this.全部启用checkBox.Checked)
                        item.IsOutput = true;
                }
                this.数据写入dataGridView.Refresh();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        private void 全部禁用checkBox_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (!this.全部禁用checkBox.Checked) return;
                this.全部启用checkBox.Checked = false;
                BindingList<WriteCommunicateCommand> WriteDataList = ((BindingList<WriteCommunicateCommand>)((SendResultSocket)this._function).DataList);
                foreach (var item in WriteDataList)
                {
                    if (this.全部禁用checkBox.Checked)
                        item.IsOutput = false;
                }
                this.数据写入dataGridView.Refresh();
            }
