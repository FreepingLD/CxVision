...
{
    public partial class WriteDataSocketForm : Form
    {
        ...
        public WriteDataSocketForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public WriteDataSocketForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<WriteCommunicateCommand > WriteDataList = ((BindingList<WriteCommunicateCommand >)((WriteDataSocket)this._function).DataList);
