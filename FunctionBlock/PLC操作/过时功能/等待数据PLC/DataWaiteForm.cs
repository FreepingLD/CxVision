...
{
    public partial class DataWaiteForm : Form
    {
        ...
        public DataWaiteForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public DataWaiteForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void DataWaiteForm_Load(object sender, EventArgs e)
        {
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                ////////////////////////////////////
                this.数据读取dataGridView.DataSource = ((DataWaitePlc)this._function).ResultInfo;
            }
            catch (Exception ee)
            {
        ...
        private void 数据读取dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<ReadCommunicateCommand> ReadDataList = ((BindingList<ReadCommunicateCommand>)((DataWaitePlc)this._function).ResultInfo);
                    switch (数据读取dataGridView.Columns[e.ColumnIndex].Name)
                    {
                        case "DeleteBtn":
                            ...
                            break;
                        case "ReadBtn":
                            ReadDataList[e.RowIndex].ReadValue = CommunicationConfigParamManger.Instance.ReadValue(ReadDataList[e.RowIndex].CoordSysName, ReadDataList[e.RowIndex].CommunicationCommand).ToString();
                            break;
                        case "InsertBtn":
