...
namespace FunctionBlock
{
    public partial class SendDataForm : Form
    {
        ...
        public SendDataForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public SendDataForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void BindProperty()
        {
            try
            {
                ...
                //////////////////////////////////////////////////   
                this.数据写入dataGridView.DataSource = ((SendData)this._function).SendDataList;
            }
            catch (Exception ee)
            {
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<SendDataCommand> WriteDataList = ((BindingList<SendDataCommand>)((SendData)this._function).SendDataList);
