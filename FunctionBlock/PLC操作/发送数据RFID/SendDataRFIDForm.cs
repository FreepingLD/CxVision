...
{
    public partial class SendDataRFIDForm : Form
    {
        ...
        public SendDataRFIDForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public SendDataRFIDForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void BindProperty()
        {
            try
            {
                ...
                //// 将已有的项添加到集合中来，因为如果集合中没的这一项，将不会显示
                foreach (var item in ((SendDataRFID)this._function).SendDataList)
                {
                    if (!MemoryManager.Instance.ContainsKey(item.DataSource))
                        this.DataColumn.Items.Add(item.DataSource);
                }
                //////////////////////////////////////////////////   
                this.数据写入dataGridView.DataSource = ((SendDataRFID)this._function).SendDataList;
            }
            catch (Exception ex)
            {
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<SendDataCommand> WriteDataList = ((BindingList<SendDataCommand>)((SendDataRFID)this._function).SendDataList);
                    switch (数据写入dataGridView.Columns[e.ColumnIndex].Name)
                    {
                        case "DeleteBtn": //InsertBtn
                            ...
                            break;
                        case "InsertBtn": //
                            WriteDataList.Insert(e.RowIndex, new SendDataCommand());
                            break;
                        case "UpMoveCol":
                            if (e.RowIndex > 0)
                            {
                                SendDataCommand up = WriteDataList[e.RowIndex - 1];
                                SendDataCommand cur = WriteDataList[e.RowIndex];
                        ...
                        case "DownMoveCol":
                            ...
                            {
                                SendDataCommand down = WriteDataList[e.RowIndex + 1];
                                SendDataCommand cur = WriteDataList[e.RowIndex];
                                WriteDataList[e.RowIndex + 1] = cur;
