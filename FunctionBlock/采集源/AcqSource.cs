...
namespace FunctionBlock
{
    ...
    public class AcqSource
    {
        ...
        public Dictionary<enDataItem, object> AcqImageData(BindingList<LightParam> lightParam, double expose = -1, bool isStopAcq = false)
        {
            bool lockTaken = false;
            Dictionary<enDataItem, object> list = new Dictionary<enDataItem, object>();
            list.Add(enDataItem.Image, null);
            ...
            {
                // 只有在实时采集下才打开关源
                if (this.Sensor.CameraParam.TriggerMode == enUserTrigerMode.NONE)
                {
                    if (this.LightParamList != null && this.LightParamList.Count > 0) // 采集源配置的参数
                    {
                        foreach (var item in this.LightParamList)
                        {
                            item.Open();
                ...
                /////////////////////////////////////
                // 设置相机曝光参数
                if (expose > 0 && expose != this.Exposure)
                {
                    this.Exposure = expose;
                    this.Sensor.SetParam("曝光", this.Exposure);
                }
                Thread.Sleep(this.Sensor.CameraParam.WaiteTime);
                LoggerHelper.Info(this.Name + "->开始触发采图");
                this.StartTrigger();
                this.StopTrigger();
                LoggerHelper.Info(this.Name + "->停止触发采图");
                list = this.getData(); // this.Sensor.ReadData();
            }
            catch (Exception ex)
            {
                LoggerHelper.Error(this.Name + "->数据采集失败", ex);
        ...
        public Dictionary<enDataItem, object> AcqPointData(MoveCommandParam moveParam)
        {
            ...
            try
            {
                ...
                if (this.Card != null)
                    this.Card.MoveMultyAxis(moveParam);        //moveParam.moveAxis, moveParam.moveSpeed, moveParam.targetPosition
                LoggerHelper.Info(this.Name + "->运动到扫描起点:" + moveParam.ToString());
        ...
        public Dictionary<enDataItem, object>[] AcqPointDatas(BindingList<CoordSysAxisPosParam> AcqCoordPointList, userWcsCoordSystem wcsCoordSystem)
        {
            bool lockTaken = false;
            ...
            try
            {
                ...
                foreach (var items in AcqCoordPointList)
                {
                    ...
                    CommandParam.AxisParam = new CoordSysAxisPosParam(items.X, items.Y, items.Z, items.Theta, items.U, items.V);
        ...
        public Dictionary<enDataItem, object> AcqScanData(MoveCommandParam moveParam1, MoveCommandParam moveParam2)
        {
            ...
            try
            {
                ...
                LoggerHelper.Info(this.Name + "->运动到扫描起点:" + moveParam1.ToString());
                this.StartTrigger();
                this.SetIoOutput(true);
                LoggerHelper.Info(this.Name + "->开始触发");
                if (this.Card != null)
                    this.Card.MoveMultyAxis(moveParam2); //moveParam2.moveAxis, moveParam2.moveSpeed, moveParam2.targetPosition
                LoggerHelper.Info(this.Name + "->运动到扫描终点:" + moveParam2.ToString());
                this.SetIoOutput(false);
                this.StopTrigger();
                LoggerHelper.Info(this.Name + "->停止触发");
                list = this.Sensor.ReadData();
        ...
        public Dictionary<enDataItem, object>[] AcqScanDatas(BindingList<userWcsLine> _AcqCoordLine, userWcsCoordSystem wcsCoordSystem)
        {
            ...
            try
            {
                ...
                foreach (var items in _AcqCoordLine)
                {
                    ...
                    CommandParam1.AxisParam = new CoordSysAxisPosParam(items.X1, items.Y1, items.Z1, items.Theta1, items.U1, items.V1);
                    ...
                    CommandParam2.AxisParam = new CoordSysAxisPosParam(items.X2, items.Y2, items.Z2, items.Theta2, items.U2, items.V2);
                    ...
                    Thread.Sleep(this.Sensor.CameraParam.WaiteTime);
                    LoggerHelper.Info(this.Name + "->运动到扫描起点:" + AffineCommandParam1.ToString());
                    this.StartTrigger();
                    this.SetIoOutput(true);
                    LoggerHelper.Info(this.Name + "->开始触发");
                    if (this.Card != null)
                        this.Card.MoveMultyAxis(AffineCommandParam2);
                    LoggerHelper.Info(this.Name + "->运动到扫描终点:" + AffineCommandParam2.ToString());
                    this.SetIoOutput(false);
                    this.StopTrigger();
                }
                ////////////////////////////////////////////
        ...
        public Dictionary<enDataItem, object> getData()
        {
            ...
            //        switch (this.Sensor.LaserParam.AcqMode)
            //        {
            //            case enAcqMode.Socket:
            //                //SocketBase socket = SocketConnectManager.Instance.GetSocket(this.SocketName);
            //                //if (socket != null)
            //                //{
            //                //    SocketMessage message = socket.GetDataAsync() as SocketMessage;
            //                //    list.Add(enDataItem.Image, message?.Message);
            //                //}
            //                break;
            //            default:
            //                list = this.Sensor.ReadData();
