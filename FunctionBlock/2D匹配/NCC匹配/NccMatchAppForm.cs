...
{
    public partial class NccMatchAppForm : Form
    {
        ...
        public NccMatchAppForm(TreeNode node)
        {
            InitializeComponent();
        ...
        public NccMatchAppForm(TreeNode node,HWindowControl hWindowControl)
        {
            InitializeComponent();
            this._function = node.Tag as IFunction;
        ...
        private void NccMatchForm_Load(object sender, EventArgs e)
        {
            BindProperty();
            //////////        
            AddForm(this.模型参数panel, new CreateNccModelParamForm(((FunctionBlock.NccModelMatch)this._function).DoNccModel.CreateNccModelParam));
            AddForm(this.搜索参数groupBox, new FindNccModelParamForm(((FunctionBlock.NccModelMatch)this._function).DoNccModel.FindNccModelParam));
            //this.AutoForm();
        }
        ...
        private void 设置搜索区域button_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (((NccModelMatch)this._function).DoNccModel.FindNccModelParam.SearchRegion.Count == 0)
                    ((NccModelMatch)this._function).DoNccModel.FindNccModelParam.SearchRegion.Add(new ModelParam(null));
                this.搜索区域dataGridView_CellContentClick(null, new DataGridViewCellEventArgs(0, 0));
            }
            catch (Exception ex)
            {
                new Common.UserMessageForm(ex.ToString()).ShowDialog();
        ...
        private void 设置模型区域button_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (((NccModelMatch)this._function).DoNccModel.CreateNccModelParam.TemplateRegion.Count == 0)
                    ((NccModelMatch)this._function).DoNccModel.CreateNccModelParam.TemplateRegion.Add(new ModelParam(null));
                this.模型区域dataGridView_CellContentClick(null, new DataGridViewCellEventArgs(0, 0));
            }
            catch (Exception ex)
            {
                new Common.UserMessageForm(ex.ToString()).ShowDialog();
        ...
        private void 创建模型button_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (((FunctionBlock.NccModelMatch)this._function).DoNccModel.CreateNccModelsFromImage(this.drawObject.BackImage.GetImage("byte")))
                {
                    this.drawObject.ClearViewObject();
