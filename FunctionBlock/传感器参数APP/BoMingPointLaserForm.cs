...
namespace FunctionBlock
{
    ...
    public partial class BoMingPointLaserParamForm : Form
    {
        private AcqSource _acqSource;
        private ISensor _sensor;
        private string savePath;
        private PointLaserForm pointForm;
        public BoMingPointLaserParamForm(ISensor sensor)
        {
            InitializeComponent();
            this._sensor = sensor;
            this.savePath = Application.StartupPath + "\\" + "BoMingLaserParam" + "\\" + this._sensor.GetParam("传感器名称").ToString() + ".txt";
        }
        public BoMingPointLaserParamForm(AcqSource acqSource)
        {
            InitializeComponent();
            this._acqSource = acqSource;
            this._sensor = this._acqSource.Sensor;
            this.pointForm = new PointLaserForm(this._acqSource);
            this.AddForm(this.监控groupBox, this.pointForm);

            this.savePath =  Application.StartupPath + "\\" + "BoMingLaserParam" + "\\" + this._sensor.GetParam("传感器名称").ToString() + ".txt";
        }
        private void BoMingPointLaserForm_Load(object sender, EventArgs e)
        {
            _initUI();
        }
