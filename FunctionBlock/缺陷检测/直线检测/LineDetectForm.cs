...
namespace FunctionBlock
{
    public partial class LineDetectForm : Form
    {
        ...
        public LineDetectForm(TreeNode node)
        {
            InitializeComponent();
        ...
        protected  void BindProperty()
        {
            try
            {
                ...
                LineDetectParam param = ((LineDetect)this._function).DetectParam as LineDetectParam;
                ...
                // this.最大轮廓距离comboBox.DataBindings.Add(this.最大轮廓距离comboBox.Text, param, nameof(param.MaxClosureDist), true, DataSourceUpdateMode.OnPropertyChanged);
                ...
                /////////////////////////////////////////////////////////
                this.距离阈值comboBox.DataBindings.Add(nameof(this.距离阈值comboBox.Text), param, nameof(param.DistThreshold), true, DataSourceUpdateMode.OnPropertyChanged);
                this.最小点数comboBox.DataBindings.Add(nameof(this.最小点数comboBox.Text), param, nameof(param.NgPointNum), true, DataSourceUpdateMode.OnPropertyChanged);
                this.相邻点距离comboBox.DataBindings.Add(nameof(this.相邻点距离comboBox.Text), param, nameof(param.PToPDist), true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////////////////
                this.单位comboBox.DataBindings.Add(nameof(this.单位comboBox.Text), param, nameof(param.Unit), true, DataSourceUpdateMode.OnPropertyChanged);
                this.像素当量comboBox.DataBindings.Add(nameof(this.像素当量comboBox.Text), param, nameof(param.PixScale), true, DataSourceUpdateMode.OnPropertyChanged);
                this.NG区域宽度comboBox.DataBindings.Add(nameof(this.NG区域宽度comboBox.Text), param, nameof(param.NgWidth), true, DataSourceUpdateMode.OnPropertyChanged);
                this.NG区域高度comboBox.DataBindings.Add(nameof(this.NG区域高度comboBox.Text), param, nameof(param.NgHeight), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
            ...
            try
            {
                if (e.DataContent != null) // 在图形窗口只显示世界坐标元素，像素元素在图像窗口显示
                {
                    switch (e.DataContent.GetType().Name) //这里只接受XLD轮廓或3D对象轮廓
                    {
                        ...
                        case "userWcsLine":
                            //this.drawObject.ClearViewObject();
                            this.drawObject.AddViewObject(new ViewData(((userWcsLine)e.DataContent).GetPixLine().GetXLD(), "green"));
                            //this.drawObject.AttachPropertyData.Clear();
