...
namespace FunctionBlock
{
    public partial class ScriptToolForm : Form
    {
        ...
        public ScriptToolForm(TreeNode node)
        {
        ...
        private void ImageReduceForm_Load(object sender, EventArgs e)
        {
            ...
            BindProperty();
            //////////////////////////////////////////////////////////
            this.addDataGridViewContextMenu(this.输入参数dataGridView);
        }
        private void BindProperty()
        {
            try
            {
                this.ParamTypeCol.Items.Clear();
                this.ParamTypeCol.ValueType = typeof(enHParamType);
                foreach (enHParamType item in Enum.GetValues(typeof(enHParamType)))
                {
                    this.ParamTypeCol.Items.Add(item);
                }
                this.OutParamTypeCol.Items.Clear();
                this.OutParamTypeCol.ValueType = typeof(enHParamType);
                foreach (enHParamType item in Enum.GetValues(typeof(enHParamType)))
                {
                    this.OutParamTypeCol.Items.Add(item);
                }
                this.输入参数dataGridView.TopLeftHeaderCell.Value = "序号";
                this.输入参数dataGridView.DataSource = ((ScriptTool)this._function).CrParam.InParam;
                this.输出参数dataGridView.TopLeftHeaderCell.Value = "序号";
                this.输出参数dataGridView.DataSource = ((ScriptTool)this._function).CrParam.OutParam;
                ...
                ScriptParam param = ((ScriptTool)this._function).CrParam as ScriptParam;
                this.脚本类型comboBox.DataBindings.Add(nameof(this.脚本类型comboBox.SelectedItem), param, nameof(param.ScriptType), true, DataSourceUpdateMode.OnPropertyChanged);
                this.脚本路径textBox.DataBindings.Add(nameof(this.脚本路径textBox.Text), param, nameof(param.ScriptPath), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
        ...
        private void dataGridViewContextMenuStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            try
            {
                ...
                switch (name)
                {
                    case "删除":
                        ((ContextMenuStrip)sender).Close();
                        if (this.输入参数dataGridView.CurrentRow != null)
                            index = this.输入参数dataGridView.CurrentRow.Index;
                    ...
                    case "矩形阵列":
                        if (this.输入参数dataGridView.CurrentRow == null) return;
                        index = this.输入参数dataGridView.CurrentRow.Index;
                ...
                if (e.DataContent != null)
                {
                    switch (e.DataContent.GetType().Name) //这里只接受XLD轮廓或3D对象轮廓
                    {
                        ...
                        case "RegionDataClass[]":
                            ...
                            break;
                        case "RegionDataClass":
                            ViewData viewData = new ViewData(((RegionDataClass)e.DataContent).Region, "red");
                            viewData.Draw = "fill";
            ...
            try
            {
                ...
                switch (this.显示条目comboBox.SelectedItem.ToString())
                {
                    case nameof(enShowItem.源图像):
                        this.drawObject.ClearViewObject();
                        this.drawObject.BackImage = ((ScriptTool)this._function).ImageData;
                        break;
                    case nameof(enShowItem.区域):
                        this.drawObject.ClearViewObject();
                        ViewData viewData = new ViewData(((ScriptTool)this._function).FlawRegion.Region, "red");
                        viewData.Draw = "fill";
                        this.drawObject.AddViewObject(viewData);
                        break;
                }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    int index = 0;
                    PixROI pixShape;
                    if (((GapDetect)this._function).Param.ShapeParam == null)
                        ((GapDetect)this._function).Param.ShapeParam = new BindingList<GapShapeParam>();
                    BindingList<GapShapeParam> listShape = ((GapDetect)this._function).Param.ShapeParam;
                    userPixCoordSystem pixCoordSystem = ((GapDetect)this._function).PixCoordSystem;
                    switch (输入参数dataGridView.Columns[e.ColumnIndex].Name)
                    {
                        case "TeachCol":
                            if (this.输入参数dataGridView.Rows[e.RowIndex].DataBoundItem == null)
                            {
        ...
        private void dataGridView1_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                if (this.输入参数dataGridView.Columns[e.ColumnIndex].DataPropertyName == "RoiShape")
                {
                    /////////////////////////
                    e.Value = EvaluateValue(this.输入参数dataGridView.Rows[e.RowIndex].DataBoundItem, this.输入参数dataGridView.Columns[e.ColumnIndex].DataPropertyName);
                    if (e.Value != null)
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    ((ScriptTool)_function).CrParam.ScriptPath = path;
