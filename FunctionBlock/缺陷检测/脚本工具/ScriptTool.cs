...
namespace FunctionBlock
{
    ...
    public class ScriptTool : BaseFunction, IFunction
    {
        [NonSerialized]
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            HDevEngine myEngine = null;
            HDevProgramCall programCall = null;
            ...
            try
            {
                ...
                program.LoadProgram(CrParam.ScriptPath);               
                foreach (var item in CrParam.InParam) // 设置输入参数
                {
                    switch (item.ParamType)
                    {
                        case enHParamType.Double:
                            double value = 0;
                            double.TryParse(item.ParamValue, out value);
                            myEngine.SetGlobalCtrlVarTuple(item.ParamName, value);
                            break;
                        case enHParamType.Int:
                            int valueInt = 0;
                            int.TryParse(item.ParamValue, out valueInt);
                            myEngine.SetGlobalCtrlVarTuple(item.ParamName, valueInt);
                            break;
                        case enHParamType.String:
                            myEngine.SetGlobalCtrlVarTuple(item.ParamName, item.ParamValue);
                            break;
                        case enHParamType.HImage:
                        case enHParamType.HObject:
                            myEngine.SetGlobalIconicVarObject(item.ParamName, this.ImageData?.Image);
                            break;
                    }
                }
                programCall = program.Execute();
                foreach (var item in CrParam.OutParam) // 设置输出参数
                {
                    switch (item.ParamType)
                    {
                        case enHParamType.Double:
                            item.ParamValue =   programCall?.GetCtrlVarTuple(item.ParamName).D.ToString();
                            break;
                        case enHParamType.HRegion:
                            hRegion = programCall? .GetIconicVarRegion(item.ParamName);
                            item.ParamValue = hRegion.ToString();
                            break;
                    }
                }
                this.Result.Succss = true;
                //////////////////////////////////////////////////////////////////////
                this._flawRegion?.Dispose();
                this._flawRegion = new RegionDataClass(hRegion);
                this._flawRegion.CamName = this._imageData?.CamName;
                this._flawRegion.ViewWindow = this._imageData?.ViewWindow;
                this._flawRegion.Tag = this._imageData?.Tag;
                this._flawRegion.Color = enColor.red;
                //this._flawRegion.Draw = "fill";
                double area = 0;
                ...
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[2].SetValue(this.name, "执行时间(ms)", stopwatch.ElapsedMilliseconds);
                OnExcuteCompleted(this._flawRegion.CamName, this._flawRegion.ViewWindow, this.name + ".Region", this._flawRegion);
                //OnExcuteCompleted(this._flawRegion.CamName, this._flawRegion.ViewWindow, this.name + ".XLD", this._xldData);
            }
            catch (Exception ex)
            {
