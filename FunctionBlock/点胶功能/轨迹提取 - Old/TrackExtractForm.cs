...
{
    public partial class TrackExtractForm : Form
    {
        private Form form;
        ...
        public TrackExtractForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        private void 导入基准轮廓Btn_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("确定要导入基准轮廓吗?", "导入基准轮廓", MessageBoxButtons.OKCancel) == DialogResult.Cancel) return;
                    ...
                    switch (extendName)
                    {
                        case ".txt":
                            ...
                            if (Data1 != null && Data1.Length > 0)
                            {
                                ...
                                MessageBox.Show("导入基准轮廓成功!");
                            }
                            else
                                MessageBox.Show("读取的点数量长度为 0");
                            break;
                        case ".dxf":
        ...
        private void 注册基准轮廓Btn_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("确定要注册基准轮廓吗?", "注册基准轮廓", MessageBoxButtons.OKCancel) == DialogResult.Cancel) return;
                userWcsPoint[] wcsPoints = ((ContourModelMatch2D)this._function).CurTrackPoint;
                if (wcsPoints != null && wcsPoints.Length > 0)
                {
                    ...
                    this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                    this.drawObject.PointCloudModel3D = new PointCloudData(new HObjectModel3D(list_x.ToArray(), list_y.ToArray(), list_z.ToArray()));
                    MessageBox.Show("注册基准轮廓成功!");
                }
                else
                    MessageBox.Show("注册基准轮廓失败!");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
