...
namespace FunctionBlock
{
    ...
    public class TrackCompose : BaseFunction, IFunction
    {
        private string _acqSourceName;
        private userWcsPolyLine _wcsPolyLine;
        private userWcsPoint[] _camTrackPoint;
        private userWcsPoint[] _laserTrackPoint;


        [DisplayName("相机轨迹点")]
        [DescriptionAttribute("输入属性1")]
        public userWcsPoint[] CamTrackPoint
        {
            get
            {
                if (this.RefSource1.Count > 0)
                {
                    ...
                    this._camTrackPoint = listPoint.ToArray();
                    listPoint.Clear();
                }
                return this._camTrackPoint;
            }
            set
            {
                this._camTrackPoint = value;
            }
        }

        [DisplayName("激光轨迹点")]
        [DescriptionAttribute("输入属性2")]
        public userWcsPoint[] LaserTrackPoint
        {
            get
            {
                if (this.RefSource2.Count > 0)
                {
                    ...
                    this._laserTrackPoint = listPoint.ToArray();
                    listPoint.Clear();
                }
                return this._laserTrackPoint;
            }
            set
            {
                this._laserTrackPoint = value;
            }
        }
        ...
        public TrackComposeParam ComposeParam { get; set; }
        ...
        public TrackCompose(string acqSourceName)
        {
            this._acqSourceName = acqSourceName;
            this.ComposeParam = new TrackComposeParam();
            InitBindingTable();
        }
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            try
            {
                ...
                this.Result.Succss = TrackComposeMethod.ComposeTrack(this.CamTrackPoint, this.LaserTrackPoint, this.ComposeParam, out this._wcsPolyLine);
                ///////////////////////////////////////////////
                stopwatch.Stop();
                this.CreateResultInfo(4);
                ((BindingList<OcrResultInfo>)this.ResultInfo)[0].SetValue(this.name, "轨迹X", string.Join(",", this._wcsPolyLine.X.ToArray()));
                ((BindingList<OcrResultInfo>)this.ResultInfo)[1].SetValue(this.name, "轨迹Y", string.Join(",", this._wcsPolyLine.Y.ToArray()));
                ((BindingList<OcrResultInfo>)this.ResultInfo)[2].SetValue(this.name, "轨迹Z", string.Join(",", this._wcsPolyLine.Z.ToArray()));
