...
{
    public partial class TrackComposeForm : Form
    {
        private Form form;
        private IFunction _function;
        private userDrawPointROI drawObject;
        private TreeNode _refNode;
        public TrackComposeForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void BindProperty()
        {
            try
            {
                this.合并方法comBox.DataSource = Enum.GetNames(typeof(enComposeMethod));
                this.合并方法comBox.DataBindings.Add(nameof(this.合并方法comBox.SelectedItem), ((TrackCompose)this._function).ComposeParam, "ComposeMethod", true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////
                /////////////////////////
                if (((TrackCompose)this._function).WcsPolyLine != null)
                {
                    List<double> list_x = ((TrackCompose)this._function).WcsPolyLine.X;
                    List<double> list_y = ((TrackCompose)this._function).WcsPolyLine.Y;
                    List<double> list_z = ((TrackCompose)this._function).WcsPolyLine.Z;
                    List<double> list_u = ((TrackCompose)this._function).WcsPolyLine.U;
                    List<double> list_v = ((TrackCompose)this._function).WcsPolyLine.V;
                    List<double> list_w = ((TrackCompose)this._function).WcsPolyLine.W;
