...
{
    public partial class TrackTransForm : Form
    {
        private Form form;
        private IFunction _function;
        private userDrawPointROI drawObject;
        private TreeNode _refNode;
        public TrackTransForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void BindProperty()
        {
            try
            {
                this.转换方法comBox.Items.Clear();
                if (((TrackTrans)this._function).WcsPolyLine?.CamParams != null)
                {
                    foreach (var item in ((TrackTrans)this._function).WcsPolyLine.CamParams.DicMapHomMat2D)
                    {
                        this.转换方法comBox.Items.Add(item.Key);
                    }
                }
                this.转换方法comBox.DataBindings.Add(nameof(this.转换方法comBox.SelectedItem), ((TrackTrans)this._function).TransParam, "Method", true, DataSourceUpdateMode.OnPropertyChanged);
                //////////////////////
                this.dataGridView1.Rows.Clear();
                int count = ((TrackTrans)this._function).WcsPolyLine.X.Count;
                List<double> list_x = ((TrackTrans)this._function).WcsPolyLine.X;
                List<double> list_y = ((TrackTrans)this._function).WcsPolyLine.Y;
                List<double> list_z = ((TrackTrans)this._function).WcsPolyLine.Z;
                for (int i = 0; i < count; i++)
                {
                    int index = this.dataGridView1.Rows.Add(list_x[i], list_y[i], list_z[i]);
                }
            }
            catch (Exception ex)
            {
