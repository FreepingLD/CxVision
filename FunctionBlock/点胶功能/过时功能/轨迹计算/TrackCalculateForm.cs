...
namespace FunctionBlock
{
    public partial class TrackCalculateForm : Form
    {
                ...
                if (e.DataContent != null) // 在图形窗口只显示世界坐标元素，像素元素在图像窗口显示
                {
                    switch (e.DataContent.GetType().Name) //这里只接受XLD轮廓或3D对象轮廓
                    {
                        ...
                        case "userWcsVector[]":
                            ...
                            {
                                double pixLength = this.drawObject.CameraParam.TransWcsLengthToPixLength((((TrackCalculate)this._function).Param).Distance);
                                this.drawObject.AddViewObject(new ViewData(item.GetPixVector().GetXLD((int)pixLength), "yellow")); // 默认绿色
                            }
                            break;
            ...
            try
            {
                ...
                switch (this.显示条目comboBox.Text.Trim())
                {
                    case "输入轨迹":
                        this.drawObject.ClearViewObject();
                        int index = 0;
                        userWcsPoint[] wcsPoints = ((TrackCalculate)this._function).TrackPoint;
                        if (wcsPoints == null) return;
                        foreach (var item in wcsPoints)
                        {
                            if (index == 0)
                            {
                                HImage hImage = new HImage("byte", item.CamParams.CamParam.Width, item.CamParams.CamParam.Height);
                                this.drawObject.BackImage = new ImageDataClass(hImage);
                                this.drawObject.CameraParam = item.CamParams;
                            }
                            this.drawObject.AddViewObject(new ViewData(item.GetPixPoint().GetXLD()));
                            index++;
                        }
                        break;
                    case "输出轨迹":
                        ...
                        {
                            this.drawObject.AddViewObject(new ViewData(item.GetPixVector().GetXLD((int)(((TrackCalculate)this._function).Param).Distance), "yellow")); // 默认绿色
                        }
                        break;
                }
        ...
        private void 显示条目comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //                    this.drawObject.CameraParam = item.CamParams;
            //                }
            //                //this.drawObject.SetViewParam(item.CamParams.CamParam.Width, item.CamParams.CamParam.Height);
            //                this.drawObject.AddViewObject(new ViewData(item.GetPixPoint().GetXLD()));
            //                index++;
            //            }
            //            break;
        ...
        protected void TrackCalculateForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
        ...
        private void titleLabel_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            TrackCalculateForm_MouseDown(null, null);  // 用标签鼠标按下事件来代替窗体鼠标按下事件
        }
        ...
        private void TrackCalculateForm_MouseDown(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
        ...
        private void 视图工具toolStrip_ItemClicked_1(object sender, ToolStripItemClickedEventArgs e)
        {
            string name = e.ClickedItem.Name;
            switch (name)
            {
