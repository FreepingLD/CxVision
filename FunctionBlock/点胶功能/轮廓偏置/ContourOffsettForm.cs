...
{
    public partial class ContourOffsetForm : Form
    {
        protected Form form;
        ...
        public ContourOffsetForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        public void ContourOffsetForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        public enum enShowItemsContourOffset
        {
            输入轮廓,
            偏置轮廓,
            输出显示,
        ...
        protected void BindProperty()
        {
            ...
            {
                // 创建匹配参数
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(enShowItemsContourOffset));
                ContourOffsetParam offsetParam = ((ContourOffset)this._function).OffsetParam;
                this.偏置距离comboBox.DataBindings.Add("Text", offsetParam, nameof(offsetParam.OffsetDist), true, DataSourceUpdateMode.OnPropertyChanged);
                this.偏置数量comboBox.DataBindings.Add("Text", offsetParam, nameof(offsetParam.OffsetCount), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ex)
            {
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case nameof(enShowItemsContourOffset.输入轮廓):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsPoint[] wcsPoints = ((ContourOffset)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                        ...
                        break;
                    case nameof(enShowItemsContourOffset.偏置轮廓):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        /////////////////////////////////////////////////////////////////////////
                        userWcsPolyLine wcsPolyLine = ((ContourOffset)this._function).WcsPolyLine;
                        if (wcsPolyLine != null)
                        {
                    ...
                    case nameof(enShowItemsContourOffset.输出显示):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        //////////////////////////////////////////////////////////////////////////
                        HObjectModel3D hObjectModel3D1 = null, hObjectModel3D2 = null;
                        wcsPoints = ((ContourOffset)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                            double[] x = new double[wcsPoints.Length];
                            double[] y = new double[wcsPoints.Length];
                        ...
                        /////////////////////////////////////////////////////////////////////////
                        wcsPolyLine = ((ContourOffset)this._function).WcsPolyLine;
                        if (wcsPolyLine != null)
                        {
                            double[] x = new double[wcsPolyLine.X.Count];
                            double[] y = new double[wcsPolyLine.X.Count];
