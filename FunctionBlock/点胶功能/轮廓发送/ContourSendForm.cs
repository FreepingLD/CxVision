...
{
    public partial class ContourSendForm : Form
    {
        protected Form form;
        ...
        public ContourSendForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        public void ContourSendForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        public enum enShowItemsContourSend
        {
            发送轮廓,
        }
        ...
        protected void BindProperty()
        {
            ...
            {
                // 创建匹配参数
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(enShowItemsContourSend));
                ContourSendParam param = ((ContourSend)this._function).Param;
                this.视图窗口comboBox.DataSource = HWindowManage.GetKeysList();
                this.坐标系名称comboBox.DataSource = Enum.GetValues(typeof(enCoordSysName));
                this.接收地址comboBox.DataSource = Enum.GetValues(typeof(enCommunicationCommand));
                this.长度地址comboBox.DataSource = Enum.GetValues(typeof(enCommunicationCommand));
                this.视图窗口comboBox.DataBindings.Add("Text", param, nameof(param.ViewWindow), true, DataSourceUpdateMode.OnPropertyChanged);
                this.坐标系名称comboBox.DataBindings.Add("Text", param, nameof(param.CoordSysName), true, DataSourceUpdateMode.OnPropertyChanged);
                this.接收地址comboBox.DataBindings.Add("Text", param, nameof(param.Adress), true, DataSourceUpdateMode.OnPropertyChanged);
                this.长度地址comboBox.DataBindings.Add("Text", param, nameof(param.LengthAdress), true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////////////////////////////////////////////////////////////
                this.启用C角计算checkBox.DataBindings.Add(nameof(this.启用C角计算checkBox.Checked), param, nameof(param.EnableCAngleCalCulate), true, DataSourceUpdateMode.OnPropertyChanged);
                this.角度取反checkBox.DataBindings.Add(nameof(this.启用C角计算checkBox.Checked), param, nameof(param.InvertAngle), true, DataSourceUpdateMode.OnPropertyChanged);
                this.角度顺序comboBox.DataBindings.Add("Text", param, nameof(param.Orientation), true, DataSourceUpdateMode.OnPropertyChanged);
                this.角度偏移comboBox.DataBindings.Add("Text", param, nameof(param.OffsetAngle), true, DataSourceUpdateMode.OnPropertyChanged);            
                this.初始角度comboBox.DataBindings.Add("Text", param, nameof(param.InitAngle), true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                this.角度方向comboBox.DataBindings.Add("Text", param, nameof(param.NormalOrientation), true, DataSourceUpdateMode.OnPropertyChanged);
                this.插值方式comboBox.DataBindings.Add("Text", param, nameof(param.InterMethod), true, DataSourceUpdateMode.OnPropertyChanged);
                this.插值参数textBox.DataBindings.Add("Text", param, nameof(param.InterParam), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ex)
            {
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case nameof(enShowItemsContourSend.发送轮廓):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        /////////////////////////////////////////////////////////////////////////
                        userWcsPolyLine wcsPolyLine = ((ContourSend)this._function).WcsPolyLine;
                        if (wcsPolyLine != null)
                        {
        ...
        protected void ContourSendForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
