...
{
    public partial class ContourPickForm : Form
    {
        private Form form;
        ...
        public ContourPickForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void TrackPullForm_Load(object sender, EventArgs e)
        {
            this.drawObject.GrayValueInfo += new GrayValueInfoEventHandler(hWindowControl1_MouseMove);
            //BaseFunction.ExcuteCompleted += new ExcuteCompletedEventHandler(DisplayExcuteResult);
            //////////////////////////////////////
            this.acqSource = AcqSourceManage.Instance.GetAcqSource(((ContourPick)this._function).AcqSourceName);
        ...
        private void BindProperty()
        {
            try
            {
                this.捨取方法comboBox.DataSource = Enum.GetNames(typeof(enPullMethod));
                this.相机采集源comboBox.DataSource = FunctionBlock.AcqSourceManage.Instance.GetAcqSourceName();
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), (ContourPick)this._function, "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////
                this.dataGridView1.DataSource = ((ContourPick)this._function).TrackPoint;
        ...
        private void TrackPickForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                this.cts?.Cancel();
        ...
        private void 视图工具toolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (name)
            {
                ...
                case nameof(toolStripButton_3D):
                    ...
                    this.toolStripButton_Translate.CheckState = CheckState.Unchecked;
                    break;
                case nameof(this.toolStripButton_Live):
                    if (this.toolStripButton_Live.Text == "开始实时")
                    {
                        this.toolStripButton_Live.ForeColor = System.Drawing.Color.Red;
                        this.toolStripButton_Live.Text = "停止实时";
                        this.toolStripButton_Live.Image = FunctionBlock.Properties.Resources.停止实时;
                        this.drawObject.IsLiveState = true;
                        Task.Run(() =>
                        {
                            while (this.drawObject.IsLiveState)
                            {
                    ...
                    else
                    {
                        ...
                        this.toolStripButton_Live.ForeColor = System.Drawing.SystemColors.ControlText;
                        this.toolStripButton_Live.Text = "开始实时";
                        this.toolStripButton_Live.Image = FunctionBlock.Properties.Resources.开始实时;
                    }
                    break;
                default:
                    break;
        ...
        private void 轨迹取点Btn_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.相机采集源comboBox.SelectedItem == null || this.相机采集源comboBox.SelectedItem.ToString() == "NONE")
                {
                    new UserMessageForm().ShowDialog("请先选择相机!");
                    return;
                }
                //////////////////////////////////////////////////
                ...
                {
                    new UserMessageForm().ShowDialog("未获取到图像!");
                    return;
                }
                //}
                enCoordSysName coordSysName = AcqSourceManage.Instance.GetAcqSource(this.相机采集源comboBox.SelectedItem.ToString()).CoordSysName;
                CoordSysAxisPosParam axisParam = new CoordSysAxisPosParam(coordSysName);
                this.drawObject.BackImage.Grab_X = axisParam.X;
                this.drawObject.BackImage.Grab_Y = axisParam.Y;
                this.drawObject.BackImage.Grab_Z = axisParam.Z;
                this.drawObject.BackImage.Grab_Theta = axisParam.Theta;
                this.drawObject.BackImage.Grab_U = axisParam.U;
                this.drawObject.BackImage.Grab_V = axisParam.V;
                ////////////////////////////////////////////////////////////////////////
                switch (this.捨取方法comboBox.Text)
                {
                    case nameof(enPullMethod.图像中心): // 图像十字中心取点示教
                        if (this.drawObject.BackImage != null)
                        {
                            this.pixPoint = new drawPixPoint(this.drawObject.BackImage.Height * 0.5, this.drawObject.BackImage.Width * 0.5);
                            this.wcsPoint = pixPoint.GetWcsPoint(this.drawObject.CameraParam, axisParam.X, axisParam.Y);
                            this.drawObject.ClearViewObject();
                            this.drawObject.AddViewObject(new ViewData(pixPoint, "green"));
                            ((ContourPick)this._function).TrackPoint.Add(new userWcsTrackPoint
                                (
                                this.wcsPoint.X,
                                this.wcsPoint.Y,
                                this.wcsPoint.Z,
                                this.drawObject.BackImage.Grab_Theta,
                                this.drawObject.BackImage.Grab_U,
                                this.drawObject.BackImage.Grab_V,
                                this.drawObject.BackImage.Grab_X,
                                this.drawObject.BackImage.Grab_Y,
                                this.drawObject.BackImage.Grab_Z,
                                this.drawObject.BackImage.Grab_Theta,
                                this.drawObject.BackImage.Grab_U,
                                this.drawObject.BackImage.Grab_V
                                ));
                        ...
                        break;
                    case nameof(enPullMethod.手动捨取): // 图像上任意取点示教
                        ///////////////////////////////////
                        if (this.drawObject.BackImage != null)
                        {
                            double row, col;
                            int btn;
                            this.hWindowControl1.HalconWindow.GetMbuttonSubPix(out row, out col, out btn);
                            this.pixPoint = new drawPixPoint(row, col);
                            ...
                            this.wcsPoint = pixPoint.GetWcsPoint(this.drawObject.CameraParam, axisParam.X, axisParam.Y);
                            this.drawObject.AddViewObject(new ViewData(pixPoint, "green")); //.GetXLD()
                            ((ContourPick)this._function).TrackPoint.Add(new userWcsTrackPoint
                               (
                                 this.wcsPoint.X,
                                 this.wcsPoint.Y,
                        ...
                        break;
                    case nameof(enPullMethod.机械坐标): // 针头示教
                        ((ContourPick)this._function).TrackPoint.Add(new userWcsTrackPoint
                            (
                            axisParam.X,
                            axisParam.Y,
                ...
                switch (name)
                {
                    ...
                    case "删除":
                        if (listShape.Count > this.dataGridView1.CurrentRow.Index)
                        {
                            ...
                            this.drawObject?.RemoveViewObjectAt(this.dataGridView1.CurrentRow.Index);
        ...
        private void 保存轨迹button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (filePath != null)
                {
                    file.WriteCSV(filePath, ((ContourPick)this._function).TrackPoint);
                }
            }
            catch (Exception ex)
            {
        ...
        private void ImageAcqCompleteEvent(object send, ImageAcqCompleteEventArgs e)
        {
        ...
        private void 相机采集源comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ((ContourPick)this._function).AcqSourceName = this.相机采集源comboBox.SelectedItem.ToString();
                this.acqSource = AcqSourceManage.Instance.GetAcqSource(((ContourPick)this._function).AcqSourceName);
                this._sensor = this.acqSource.Sensor;
            }
            catch (Exception ex)
