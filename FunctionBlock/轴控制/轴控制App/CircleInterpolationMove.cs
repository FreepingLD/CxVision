...
namespace FunctionBlock
{
    ...
    /// 激光取点采集的数据类
    /// </summary>
    public class CircleInterpolationMove : BaseFunction, IFunction
    {
        // 从机台读取的坐标值
        ...
        public CircleInterpolationMove(IMotionControl _card)
        {
            if (_card != null)
                this._card = MotionCardManage.GetCard(_card.Name);//  new AcqSource(_camSensor);
            this.Coord1Table.Columns.AddRange(new DataColumn[4] { new DataColumn("圆心X坐标"), new DataColumn("圆心Y坐标"), new DataColumn("终点X坐标"), new DataColumn("终点Y坐标") });
        }
        public CircleInterpolationMove(IMotionControl _card, IFunction coordSystem)
        {
            if (_card != null)
                this._card = MotionCardManage.GetCard(_card.Name);//  new AcqSource(_camSensor);
            if (coordSystem != null)
                ...
                for (int i = 0; i < dataCoord1Row.Length; i++) //
                {
                    ...
                    switch (this.moveAxis)
                    {
                        case enAxisName.XY轴圆弧插补:
                            CommandParam.AxisParam = new CoordSysAxisParam(); //new double[2] { Convert.ToDouble(dataCoord1Row[i]["终点X坐标"]),Convert.ToDouble(dataCoord1Row[i]["终点Y坐标"]) };
                            this.CommandParam.CircleParam.centerPosition = new double[2] { Convert.ToDouble(dataCoord1Row[i]["圆心X坐标"]), Convert.ToDouble(dataCoord1Row[i]["圆心Y坐标"]) };
                            this._card.MoveMultyAxis(CommandParam);
                            break;
                        case enAxisName.XYZ轴圆弧插补:
                            CommandParam.AxisParam = new CoordSysAxisParam();//new double[3] { Convert.ToDouble(dataCoord1Row[i]["终点X坐标"]), Convert.ToDouble(dataCoord1Row[i]["终点Y坐标"]), Convert.ToDouble(dataCoord1Row[i]["终点Z坐标"]) };
                            this.CommandParam.CircleParam.centerPosition = new double[3] { Convert.ToDouble(dataCoord1Row[i]["圆心X坐标"]), Convert.ToDouble(dataCoord1Row[i]["圆心Y坐标"]), Convert.ToDouble(dataCoord1Row[i]["圆心Z坐标"]) };
                            this._card.MoveMultyAxis(CommandParam);
                            break;
                        default:
