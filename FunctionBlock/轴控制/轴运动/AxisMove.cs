...
namespace FunctionBlock
{
    ...
    public class AxisMove : BaseFunction, IFunction
    {
        // 从机台读取的坐标值
        ...
        private enAxisName moveAxis = enAxisName.X轴;
        private ushort ioOutPort = 0;
        private bool asynchronous = false;
        private MoveCommandParam CommandParam = new MoveCommandParam(enAxisName.X轴, 10);
        ...
        [DisplayName("运动参数")]
        [Description("输出参数")]
        public AxisMoveParam MoveParam { get; set ; }


        public AxisMove(IMotionControl _card)
        {
            if (_card != null)
                this._card = MotionCardManage.GetCard(_card.Name);//  new AcqSource(_camSensor);
            this.Coord1Table.Columns.Add(new DataColumn("X坐标"));
        }
        public AxisMove(IMotionControl _card, IFunction coordSystem)
        {
            if (_card != null)
                this._card = MotionCardManage.GetCard(_card.Name);//  new AcqSource(_camSensor);
            if (coordSystem != null)
