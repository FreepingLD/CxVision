...
{
    public partial class BilateralFilterForm : Form
    {
        private FilterParam _param;
        public BilateralFilterForm(FilterParam param)
        {
            this._param = param;
            InitializeComponent();
            BindProperty();
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.启用滤波checkBox.DataBindings.Add(nameof(this.启用滤波checkBox.Checked), (BilateralFilterParam)this._param, "IsFilter", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
