...
namespace FunctionBlock
{
    public partial class CameraParamForm : Form
    {
        ...
        public CameraParamForm(CameraParam cameraParam)
        {
            ...
            this._acqParam = new ImageAcqParam();
            this.Text = $"{this.Text}({this._cameraParam.SensorName})";
            this._sensor = SensorManage.GetSensor(this._cameraParam.SensorName);
            ///// 从网络中读取相机参数
            if (this._sensor != null && this._sensor.ConfigParam.ConnectType == enUserConnectType.Socket)
            {
                this._socket = Common.SocketConnectManager.Instance.GetSocket(this._sensor?.ConfigParam.ConnectAddress);
                SocketMessage message1 = new SocketMessage(enSocketInfo.读取相机参数, this._sensor?.Name);
                message1.Name = this._sensor?.Name;
                this._cameraParam = this._socket.GetDataAsync(message1, true) as CameraParam;
            }
        }
        public CameraParamForm(CameraParam cameraParam, ImageAcqParam acqParam)
        {
            ...
            if (this._sensor != null && this._sensor.ConfigParam.ConnectType == enUserConnectType.Socket)
            {
                ...
                SocketMessage message1 = new SocketMessage(enSocketInfo.读取相机参数, this._sensor?.Name);
                message1.Name = this._sensor?.Name;
                object recive = this._socket?.GetDataAsync(message1, true);
                switch (recive?.GetType().Name)
                {
                    case nameof(CameraParam):
                        this._cameraParam = recive as CameraParam;
                        break;
                    case nameof(SocketMessage):
                        message1 = recive as SocketMessage;
                        this._cameraParam = message1.MesContent as CameraParam;
                        break;
                    default:
        ...
        private void CameraParamMangerForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.坐标映射方法comboBox.Items.Clear(); // 更新键值
                this.坐标映射方法comboBox.Items.Add("NONE");
                foreach (var item in this._cameraParam.DicMapHomMat2D)
                    this.坐标映射方法comboBox.Items.Add(item.Key);
                foreach (var item in this._cameraParam.DicMapHomMat3D)
                    this.坐标映射方法comboBox.Items.Add(item.Key);
                /////////////////////////////////////////////////////
                ...
                this.像素尺寸textBox.DataBindings.Add("Text", this._cameraParam, nameof(this._cameraParam.PixScale), true, DataSourceUpdateMode.OnPropertyChanged);
                ...
                this.启用倾斜校正checkBox.DataBindings.Add("Checked", this._cameraParam, nameof(this._cameraParam.EnableSlantRectify), true, DataSourceUpdateMode.OnPropertyChanged);
                this.启用平场校正checkBox.DataBindings.Add("Checked", this._cameraParam, nameof(this._cameraParam.EnableFlatRectify), true, DataSourceUpdateMode.OnPropertyChanged);
                this.倾斜轴comboBox.DataBindings.Add("Text", this._cameraParam, nameof(this._cameraParam.SlantAxis), true, DataSourceUpdateMode.OnPropertyChanged);
                this.倾斜角度textBox.DataBindings.Add("Text", this._cameraParam, nameof(this._cameraParam.CamSlant), true, DataSourceUpdateMode.OnPropertyChanged);
                ...
                //this.采集延时textBox.DataBindings.Add("Text", this._cameraParam, nameof(this._cameraParam.AcqWaiteTime), true, DataSourceUpdateMode.OnPropertyChanged); // 激光才有 
                ...
                this.图像索引comboBox.DataBindings.Add("Text", this._cameraParam, nameof(this._cameraParam.ImageIndex), true, DataSourceUpdateMode.OnPropertyChanged);
                this.图像类型comboBox.DataBindings.Add("Text", this._cameraParam, nameof(this._cameraParam.ImageType), true, DataSourceUpdateMode.OnPropertyChanged);
                this.坐标映射方法comboBox.DataBindings.Add(nameof(this.坐标映射方法comboBox.Text), this._cameraParam, nameof(this._cameraParam.MapType), true, DataSourceUpdateMode.OnPropertyChanged);
                //////////////  附加的激光参数
                this.启用X轴checkBox.DataBindings.Add("Checked", this._cameraParam, nameof(this._cameraParam.Enable_x), true, DataSourceUpdateMode.OnPropertyChanged);
                this.启用Y轴checkBox.DataBindings.Add("Checked", this._cameraParam, nameof(this._cameraParam.Enable_y), true, DataSourceUpdateMode.OnPropertyChanged);
                ...
                if (this._acqParam.Expose > 0)
                    this.曝光textBox.Text = this._acqParam.Expose.ToString();
                //this.曝光textBox.DataBindings.Add("Text", this._acqParam, nameof(this._acqParam.Expose), true, DataSourceUpdateMode.OnPropertyChanged);
                else
                ...
                // 读取 
                //this.像素当量textBox.ReadOnly = true;
            }
            catch (Exception ex)
            {
        ...
        private void CameraParamForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                //if (SensorManage.GetSensor(this._cameraParam.SensorName).ConfigParam.ConnectType == enUserConnectType.Socket)
                //{
                //    SocketBase socket1 = SocketConnectManager.Instance.GetSocket(SensorManage.GetSensor(this._cameraParam.SensorName).ConfigParam.ConnectAddress);
                //    if (socket1 != null)
                //    {
                //        if (new UserMessageForm().ShowDialog("是否同步更新服务器参数?", "更新参数") == DialogResult.OK)
                //        {
                //            SocketMessage message = new SocketMessage(enSocketInfo.写入相机参数, this._cameraParam.SensorName);
                //            message.Name = this._cameraParam.SensorName;
                //            object result = socket1.GetDataAsync(message, true);
                //            if (result.ToString().Trim() == "OK")
                //                new UserMessageForm().ShowDialog("更新服务器相机参数成功!");
                //            else
                //                new UserMessageForm().ShowDialog("更新服务器相机参数失败!");
                //        }
                //    }
                //}
            }
        ...
        private void exposeBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this._cameraParam.Exposure = exposure;
                if (this._acqParam.Expose > 0)
                    this._acqParam.Expose = exposure;
                else
                    SensorManage.GetSensor(this._cameraParam.SensorName).SetParam("曝光", 曝光textBox.Text);
            }
            catch (Exception ex)
