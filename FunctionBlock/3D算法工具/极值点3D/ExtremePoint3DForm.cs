...
{
    public partial class ExtremePointForm3D : Form
    {
        ...
        public ExtremePointForm3D(IFunction function,TreeNode node)
        {      
            this._function = function;
            InitializeComponent();
            this.Text = ((ExtremePoint3D)this._function).Name;
        ...
        private void ExtremePointForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(ExtremePoint3D.enShowItems));
                //this.dataGridView1.DataSource = ((ExtremePoint3D)this._function).ResultDataTable;
                this.极值点坐标comboBox.DataSource = Enum.GetNames(typeof(enExtremePointCoord));
                this.极值点模式comboBox.DataSource = Enum.GetNames(typeof(enExtremePointMode));
                this.平均数textBox.DataBindings.Add("Text", (ExtremePoint3D)this._function, "OffsetValue", true, DataSourceUpdateMode.OnPropertyChanged);
                this.极值点坐标comboBox.DataBindings.Add("Text", (ExtremePoint3D)this._function, "ExtremePointCoord", true, DataSourceUpdateMode.OnPropertyChanged);
                this.极值点模式comboBox.DataBindings.Add("Text", (ExtremePoint3D)this._function, "ExtremePointMode", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
        ...
        private void ExtremePointForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
