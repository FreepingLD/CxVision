...
{
    public partial class CreatePrimitive3DForm : Form
    {
        ...
        public CreatePrimitive3DForm(IFunction function,TreeNode node)
        {      
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.基本体类型comboBox.DataSource = Enum.GetNames(typeof(enPrimitiveType));
                this.基本体类型comboBox.DataBindings.Add("Text", (FunctionBlock.CreatePrimitive3D)this._function, "PrimitiveType", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
        ...
        private void 基本体类型comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (form != null) form.Dispose();
            if (this.基本体类型comboBox.Text == "" || this.基本体类型comboBox.SelectedIndex == -1) return;
            switch (this.基本体类型comboBox.SelectedItem.ToString().Trim())
            {
                case "gen_plane_object_model_3d":
                    if(this.isLoad)
                    ((CreatePrimitive3D)this._function).Param = new PlanePrimitiveParam();
                    form = new CreatePlaneForm(((CreatePrimitive3D)this._function).Param);
                    AddForm(this.groupBox3, form);
                    break;
                case "gen_sphere_object_model_3d":
                    if (this.isLoad)
                        ((CreatePrimitive3D)this._function).Param = new SpherePrimitiveParam();
                    form = new CreateSphereForm(((CreatePrimitive3D)this._function).Param);
                    AddForm(this.groupBox3, form);
                    break;
                case "gen_cylinder_object_model_3d":
                    if (this.isLoad)
                        ((CreatePrimitive3D)this._function).Param = new CylinderPrimitiveParam();
                    form = new CreateCylinderForm(((CreatePrimitive3D)this._function).Param);
                    AddForm(this.groupBox3, form);
                    break;
                case "gen_box_object_model_3d":
                    if (this.isLoad)
                        ((CreatePrimitive3D)this._function).Param = new BoxPrimitiveParam();
                    form = new CreateBoxForm(((CreatePrimitive3D)this._function).Param);
                    AddForm(this.groupBox3, form);
                    break;
            }
        }
        private void CreatePrimitive3DForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (this.cts != null)
