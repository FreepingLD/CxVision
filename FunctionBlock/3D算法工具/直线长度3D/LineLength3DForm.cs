...
{
    public partial class LineLength3DForm : Form
    {
        ...
        public LineLength3DForm(IFunction function,TreeNode node)
        {      
        ...
        private void BindProperty()
        {
            try
            {
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(FunctionBlock.LineLength3D.enShowItems));
               // this.dataGridView1.DataSource = ((FunctionBlock.LineLength3D)this._function).ResultDataTable;
                this.标准值textBox.DataBindings.Add("Text", (FunctionBlock.LineLength3D)this._function, "StdValue", true, DataSourceUpdateMode.OnPropertyChanged);
                this.上公差textBox.DataBindings.Add("Text", (FunctionBlock.LineLength3D)this._function, "UpTolerance", true, DataSourceUpdateMode.OnPropertyChanged);
                this.下公差textBox.DataBindings.Add("Text", (FunctionBlock.LineLength3D)this._function, "DownTolerance", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
                MessageBox.Show("BindProperty()执行错误" + ee.ToString());
            }
        }
        ...
        private void LineOrientationForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
