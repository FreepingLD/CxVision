...
{
    public partial class TransformXYZImageToObject3DForm : Form
    {
        ...
        public TransformXYZImageToObject3DForm(IFunction function,TreeNode node)
        {      
        ...
        private void BindProperty()
        {
            try
            {
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(TransformXYZImageToObject3D.enShowItems));
                //this.拟合基本体comboBox.DataSource = Enum.GetNames(typeof(CalculateObjectPose3D.enPrimitiveType));
                //this.拟合算法comboBox.DataSource = Enum.GetNames(typeof(CalculateObjectPose3D.enFitAlgorithm));
                //// this.结果类型comboBox.DataSource = Enum.GetNames(typeof(FittingPrimitive3D.enResultType));
                //this.拟合基本体comboBox.DataBindings.Add("Text", (CalculateObjectPose3D)this._function, "PrimitiveType", true, DataSourceUpdateMode.OnPropertyChanged);
                //this.拟合算法comboBox.DataBindings.Add("Text", (CalculateObjectPose3D)this._function, "FittingAlgorithm", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
