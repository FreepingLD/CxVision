...
{
    public partial class FittingProfileCircle3DForm : Form
    {
        ...
        public FittingProfileCircle3DForm(IFunction function,TreeNode node)
        {
        ...
        private void FittingCircle3DForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        private void BindProperty()
        {
            try
            {
                //this.dataGridView1.DataSource = ((FunctionBlock.FitProfileCircle3D)this._function).ResultDataTable;
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(enShowItems));
                this.拟合坐标comboBox.DataSource = Enum.GetNames(typeof(enFitCoordPoint));
                this.拟合算法comboBox.DataSource = Enum.GetNames(typeof(enFitCircleAlgorithm));
                this.拟合坐标comboBox.DataBindings.Add("Text", ((FunctionBlock.FitProfileCircle3D)this._function).FtiParam, "FitCoordPoint", true, DataSourceUpdateMode.OnPropertyChanged);
                this.拟合算法comboBox.DataBindings.Add("Text", ((FunctionBlock.FitProfileCircle3D)this._function).FtiParam, "FitMethod", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
