...
{
    public partial class Save3dDataForm : Form
    {
        ...
        public Save3dDataForm(IFunction function,TreeNode node)
        {      
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(SaveObjectModel3D.enShowItems));
                SaveObjectModelParam param = ((SaveObjectModel3D)this._function).SaveParam;
                this.文件目录OK_textBox.DataBindings.Add("Text", param, nameof(param.FolderPath), true, DataSourceUpdateMode.OnPropertyChanged);
                this.文件类型comboBox.DataBindings.Add("Text", param, nameof(param.ExtendName), true, DataSourceUpdateMode.OnPropertyChanged);
                this.日期checkBox.DataBindings.Add("Checked", param, nameof(param.AddDataTime), true, DataSourceUpdateMode.OnPropertyChanged);
                this.文件名称OK_TextBox.DataBindings.Add("Text", param, nameof(param.FileName), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
        ...
        private void directoryButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                fold.ShowDialog();
                this.文件目录OK_textBox.Text = fold.SelectedPath;
            }
            catch
            {
        ...
        private void 文件类型comboBox_TextChanged(object sender, EventArgs e)
        {
            if (this.文件类型comboBox.Text == "stl")
                this.注意标签label.Text = "保存stl格式必需保证对象模型中包含三角形或多边形，如果为点去对象，则必需转换成三角形对象";
            else
                this.注意标签label.Text = "";
        }
