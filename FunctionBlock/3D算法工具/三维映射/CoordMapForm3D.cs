...
{
    public partial class CoordMapForm3D : Form
    {
        ...
        public CoordMapForm3D(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public CoordMapForm3D(TreeNode node)
        {
            this._refNode = node;
        ...
        private void CoordMapForm3D_Load(object sender, EventArgs e)
        {
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                this.变换类型comboBox.DataBindings.Add("Text", ((CoordMap3D)this._function).MapParam, "TransformationType", true, DataSourceUpdateMode.OnPropertyChanged);
                ///////////
                userWcsPoint [] coordPoint1 = ((CoordMap3D)this._function).SourcePoint;
                userWcsPoint[] coordPoint2 = ((CoordMap3D)this._function).TargetPoint;
                if (coordPoint1 != null)
                {
                    for (int i = 0; i < coordPoint1.Length; i++)
                    {
                        this.P坐标dataGridView.Rows.Add(coordPoint1[i].X, coordPoint1[i].Y, coordPoint1[i].Z);
                        this.P坐标dataGridView.Rows[i].HeaderCell.Value = (i + 1).ToString();
                    }
                ...
                if (coordPoint2 != null)
                {
                    ...
                    {
                        this.Q坐标dataGridView.Rows.Add(coordPoint2[i].X, coordPoint2[i].Y, coordPoint2[i].Z);
                        this.Q坐标dataGridView.Rows[i].HeaderCell.Value = (i + 1).ToString();
                    }
                }
            }
            catch (Exception ee)
        ...
        private void CoordMapForm3D_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (cts != null)
