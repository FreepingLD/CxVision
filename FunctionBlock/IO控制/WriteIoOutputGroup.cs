...
namespace FunctionBlock
{
    ...
    /// 激光取点采集的数据类
    /// </summary>
    public class WriteIoOutputGroup : BaseFunction, IFunction
    {

        private IMotionControl _card;
        private ushort ioOutputGroup = 0;
        ...
        public ushort IoOutputGroup { get => ioOutputGroup; set => ioOutputGroup = value; }
        ...
        public WriteIoOutputGroup(IMotionControl _card)
        {
            if (_card != null)
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            try
            {
                ...
                this._card.WriteIoOutputGroup(this.ioPortType, this.ioOutputGroup, this.ioOutputlevel);
                // 使用发命令的方式来更新视图  
                Result.Succss = true;
                if (Result.Succss)
                    LoggerHelper.Info(this.name + "写入Io输出组完成");
                else
                    LoggerHelper.Error(this.name + "写入Io输出组失败");
            }
            catch (Exception e)
            {
                LoggerHelper.Error(this.name + "写入Io输出组报错" + e);
                return Result;
            }
            return Result;
        }
