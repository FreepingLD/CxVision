...
{
    public partial class CoordSystemCircleLineForm : Form
    {
        ...
        public CoordSystemCircleLineForm(IFunction function, TreeNode node)
        {
        ...
        private void CoordSystemCircleLineForm_Load(object sender, EventArgs e)
        {
            // 注册事件
            ...
            BindProperty();
            ///////////////////////////////
            this.drawObject.AttachPropertyData.Add(((FunctionBlock.CoordSystemCircleLine)this._function).PixCircle);
            this.drawObject.AttachPropertyData.Add(((FunctionBlock.CoordSystemCircleLine)this._function).PixLine);
        }
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.坐标原点comboBox.DataBindings.Add("Text", ((FunctionBlock.CoordSystemCircleLine)this._function).Param, "OrigionType", true, DataSourceUpdateMode.OnPropertyChanged);
                this.初始化参考点comboBox.DataBindings.Add("Text", ((FunctionBlock.CoordSystemCircleLine)this._function).Param, "IsInit", true, DataSourceUpdateMode.OnPropertyChanged);
                this.补正类型comboBox.DataBindings.Add("Text", ((FunctionBlock.CoordSystemCircleLine)this._function).Param, "AdjustType", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                object object3D = null; //  this._function.GetPropertyValues(this.显示条目comboBox.Text.Trim());
                switch (this.显示条目comboBox.Text.Trim())
                {
                    case nameof(enShowItems.输入对象1):
                        object3D = ((FunctionBlock.CoordSystemCircleLine)this._function).PixCircle;
                        break;
                    case nameof(enShowItems.输入对象2):
                        object3D = ((FunctionBlock.CoordSystemCircleLine)this._function).PixLine;
                        break;
                    case nameof(enShowItems.输出对象):
                        object3D = ((FunctionBlock.CoordSystemCircleLine)this._function).WcsCoordSystem;
                        break;
                }
                ///////////////////////////////////////////
                if (object3D != null)
        ...
        private void CoordSystemCircleLineForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
        ...
        private void 角度置零but_Click(object sender, EventArgs e)
        {
            try
            {
                ((FunctionBlock.CoordSystemCircleLine)this._function).Param.RefPoint_Rad = 0;
            }
            catch (Exception ex)
            {
        ...
        private void 重置坐标系but_Click(object sender, EventArgs e)
        {
            try
            {
                ((FunctionBlock.CoordSystemCircleLine)this._function).Param.IsInit = false;
