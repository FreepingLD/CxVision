...
{
    public partial class CamRotatetCalibrateSimpleForm : Form
    {
        ...
        public CamRotatetCalibrateSimpleForm()
        {
            InitializeComponent();
            this.CamParam = null;
        ...
        public CamRotatetCalibrateSimpleForm(CameraParam CamParam)
        {
            InitializeComponent();
        ...
        private void CamRotatetCalibrateSimpleForm_Load(object sender, EventArgs e)
        {
            // 注册事件
                ...
                case enCalibMethod.先旋转后平移:
                    ...
                    switch (this.CamParam.CaliParam.CalculateMethod)
                    {
                        ...
                        case enCalculateMethod.拟合圆:
                            CalibrateMethod.Instance.CalculateCenterFit(rowsList.ToArray(), colsList.ToArray(), out center_Row, out center_Col);
                        ...
                        case enCalculateMethod.矩阵变换:
                            CalibrateMethod.Instance.CalculateCenterHomMatWcs(list_x.ToArray(), list_y.ToArray(), this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep, out center_Row, out center_Col, out error);
                            rotateAngle = this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep;
                            /////////////////  计算圆心后，调整圆心 ///////
                            break;
                        ...
                        case enCalculateMethod.五点计算圆心:
                            CalibrateMethod.Instance.CalculateCenterHomMatWcs_5Point(list_x.ToArray(), list_y.ToArray(), this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep, out center_Row, out center_Col, out error);
                            rotateAngle = this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep;
                            break;
                    }
                    /////////////////  计算圆心后，调整圆心 ///////
                    double rowCenter2, colCenter2;
                    if (this.CamParam.CaliParam.IsIterationl)
                    {
                        CalibrateMethod.Instance.AdjustCenter(rowsList.ToArray(), colsList.ToArray(), rotateAngle, center_Row, center_Col, out rowCenter2, out colCenter2, out error);
                        center_Row = rowCenter2;
                        center_Col = colCenter2;
                    }
                    ////////////////////////////////////
                    this.旋转坐标dataGridView.Rows.Clear();
                    for (int i = 0; i < rowsList.Count; i++)
                    {
                        this.旋转坐标dataGridView.Rows.Add(rowsList[i], colsList[i]);
                    }
                    new UserMessageForm().ShowDialog("放转中心:" + "Row = " + center_Row.ToString("f3") + ";Col = " + center_Col.ToString("f3") + ";步长角 = " + rotateAngle.ToString("f3"));
                    // 再执行平移
                    this.listData?.Clear();
                    ...
                    //////////////////////////////////////////
                    if (new UserMessageForm().ShowDialog("标定完成,N点标定最大误差：" + error.ToString("f3") + "; 标定矩阵:" + this.CamParam.HomMat2D.ToString(), "是否更新并保存参数？") == DialogResult.OK)
                    {
                        this.CamParam?.Save();
                    }
                ...
                case enCalibMethod.仅旋转:
                    ...
                    switch (this.CamParam.CaliParam.CalculateMethod)
                    {
                        ...
                        case enCalculateMethod.矩阵变换:
                            //CalibrateMethod.Instance.CalculateCenterHomMatWcs(list_x.ToArray(), list_y.ToArray(), this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep, out Center_x, out Center_y, out error);
                            //rotateAngle = this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep;
                            if (this.CamParam.CaliParam.RotateDirection == enRotateDirection.正反向)  // 用三点来计算圆心
                            {
                                CalibrateMethod.Instance.CalculateCenterHomMatWcs_5Point(list_x.ToArray(), list_y.ToArray(), this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep, out Center_x, out Center_y, out error);
        ...
        private void CalibSocketNew()
        {
            ...
            switch (this.CamParam.CaliParam.CalibMethod)
            {
                ...
                case enCalibMethod.仅旋转:
                    ...
                    /////////////////  计算圆心后，调整圆心 ///////
                    double rowCenter = Center_x;
                    double colCenter = Center_y;
                    double radius = 0;
                    double step = 0;
                    if (this.CamParam.CaliParam.IsIterationl)
                    {
        ...
        private void CalibSocketNewXZ()
        {
            ...
            foreach (var item in this.calibCoordConfigParamRotaV.CalibCoordParamList)
            {
                ...
                else
                {
                    //this.toolStripButton_Run.Enabled = true;
                    //this.toolStripButton_Stop.Enabled = false;
                    //new UserMessageForm().ShowDialog("没有获取到可执行的节点!!");
                    //return;
                    this._treeViewWrapClassNPoint.RunSyn(null, 1);
                }
            }
        ...
        private void 生成旋转坐标button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (this.旋转方向comboBox.Text)
                {
                    ...
                    case "正反向":  // 共转三个角度,
                        ///////////////////  0 度位置
                        double stepCount = (int)(this.CamParam.CaliParam.AngleStep / 2);
                        this.calibCoordConfigParamRota.CalibCoordParamList.Add(new CoordSysAxisPosParam(
                             this.CamParam.CaliParam.RotateCalibPoint.X,
                             this.CamParam.CaliParam.RotateCalibPoint.Y,
                             this.CamParam.CaliParam.RotateCalibPoint.Z,
                        ...
                        for (int i = 1; i <= stepCount; i++)
                        {
                            this.calibCoordConfigParamRota.CalibCoordParamList.Add(new CoordSysAxisPosParam(
                                ...
                                this.CamParam.CaliParam.RotateCalibPoint.Y,
                                this.CamParam.CaliParam.RotateCalibPoint.Z,
                                this.CamParam.CaliParam.RotateCalibPoint.Angle - this.CamParam.CaliParam.AngleRange / this.CamParam.CaliParam.AngleStep * i));
                        }
                        //////////////////////  正向转N个角度  //////////////////////////////////////////////////
                        for (int i = 1; i <= stepCount; i++)
                        {
