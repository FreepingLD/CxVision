...
namespace FunctionBlock
{
    ...
    public class TreeViewWrapClass
    {
        ...
        public TreeViewWrapClass(TreeView treeView, Form form, bool isShow = false)
        {
            this.ToolName = "";
            //GlobalProgram.ProgramItems = new Dictionary<string, IFunction>();
            ...
            if (isShow)
            {
                this.treeView.ShowPlusMinus = true; // 设置节点样式
        ...
        public async void RunAsyn(ToolStripItem item, int Count) //async
        {
            ...
            try
            {
                ...
                //    LightConnectManage.CurrentLight.SetLight(enLightChannel.Channel_All,0);
                return;
            }
            catch (Exception ex)
            {
                this.RunError?.Invoke(this, new EventArgs());
                new Common.UserMessageForm("停止运行" + ex.ToString()).ShowDialog();
                //new UserMessageForm().ShowDialog("停止运行" + ex.ToString());
                return;
            }
            finally
            {
        ...
        public bool SaveProgram(string filePath)
        {
            ...
            {
                new Common.UserMessageForm(ex.ToString() + "保存程序树视图出错").ShowDialog();
                //new UserMessageForm().ShowDialog(ex.ToString() + "保存程序树视图出错");
                result = false;
            }
            //// 再将Tag节点的引用还原回去
            for (int i = 0; i < this.treeView.Nodes.Count; i++)
            ...
            foreach (KeyValuePair<string, IFunction> item in GlobalProgram.ProgramItems)
            {
                ...
                {
                    result = false;
                    new Common.UserMessageForm("保存对象：" + item.Key + "失败" + ex.ToString()).ShowDialog();
                    //new UserMessageForm().ShowDialog("保存对象：" + item.Key + "失败" + ex.ToString());
                }
            }
            return result;
        }
        ...
        public TreeNode AddItems(IFunction function, string name, params string[] propertyName)
        {
            try
            {
                ...
                if (DefaultCustomAttribute == null)
                    new Common.UserMessageForm("添加的类型没有定义默认属性，请先定义类的默认属性").ShowDialog();
                //new UserMessageForm().ShowDialog("添加的类型没有定义默认属性，请先定义类的默认属性");
                else
            ...
            {
                new Common.UserMessageForm("添加对象出错：" + ex.ToString()).ShowDialog();
                //new UserMessageForm().ShowDialog("添加对象出错：" + ex.ToString());
                return null;
            }
        }
        ...
        private Image GetImageKey(string name, ImageList imageList)
        {
            Bitmap hImage = null;
            string[] imageFile = Directory.GetFiles("", ".*");
            foreach (var item in imageFile)
            {
                if (new FileInfo(item).Name == name)
                    hImage = new Bitmap(item);
            }
            return hImage;
        }
        ...
        private void addContextMenu(TreeView treeView)
        {
            ...
            ToolStripItem[] items = new ToolStripMenuItem[]
            {
                ...
                new ToolStripMenuItem("移动到拍照位"),
                new ToolStripMenuItem("隐藏节点"),
                new ToolStripMenuItem("显示节点"),
        ...
        private void contextMenuStrip1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            try
            {
                switch (name)
                {
                    ...
                    case "执行":
                        context.Close();
                        this.node = this.treeView.SelectedNode; // 禁用的节点在手动模式下可以执行
                        item = this.treeView.SelectedNode.Tag as IFunction;
                        Task.Run(() => item?.Execute(this.node, "manual")); // manual: 表示手动模式操作
                        if (item == null)
                            new Common.UserMessageForm(this.node.Text + ":Tag对象为空引用").ShowDialog();
                        //new UserMessageForm().ShowDialog(this.node.Text + ":Tag对象为空引用");
                        break;
                    case "禁用":
                        context.Close();
                    ...
                    case "移动到拍照位":
                        ...
                        switch (item?.GetType().Name)
                        {
                            ...
                            case nameof(ManualLineMeasure):
                            case nameof(ManualCircleMeasure):
                            case nameof(ManualCircleSectorMeasure):
                                item.SetPropertyValues("Move", null);
                                break;
                        }
                        break;

                    case "隐藏节点":
                        ((BaseFunction)this.treeView.SelectedNode.Tag).IsVisualNode = false;
                        break;

                    case "显示节点":
                        ((BaseFunction)this.treeView.SelectedNode.Tag).IsVisualNode = true;
                        break;
                    ...
                    case "删除":
                        ...
                        else
                        {
                            ...
                            {
                                case "RefSource1":
                                    if ((BaseFunction)this.treeView.SelectedNode.Parent.Parent.Tag != null)
                                    {
                                        ((BaseFunction)this.treeView.SelectedNode.Parent.Parent.Tag).RefSource1.Remove(this.treeView.SelectedNode.Name);
                                        this.treeView.SelectedNode.Remove();
                                    }
                                    else
                                        new Common.UserMessageForm($"节点->{this.treeView.SelectedNode.Parent.Parent.Text}引用的对象为 NULL").ShowDialog();
                                    //new UserMessageForm().ShowDialog($"节点->{this.treeView.SelectedNode.Parent.Parent.Text}引用的对象为 NULL");
                                    break;
                                case "RefSource2":
                                    if ((BaseFunction)this.treeView.SelectedNode.Parent.Parent.Tag != null)
                                    {
                    ...
                    case "重命名":
                        ...
                        {
                            new Common.UserMessageForm("新名字中不能包含有 - 字符!!!").ShowDialog();
                            //new UserMessageForm().ShowDialog("新名字中不能包含有 - 字符!!!");
                            return;
                        }
                        if (resetNameForm.IsOk)
                        {
                            foreach (TreeNode node in this.treeView.Nodes)
                            {
                                ...
                                {
                                    new Common.UserMessageForm("已存在相同名字的节点!").ShowDialog();
                                    // new UserMessageForm().ShowDialog("已存在相同名字的节点!");
                                    return;
                                }
                            }
        ...
        private void treeView1_DragDrop(object sender, DragEventArgs e) // 在自己身上实现拖放操作
        {
            try
            {
                ...
                else  // 表示落地处的节点为子节点,表示为子节点添加元素
                {
                    ...
                    {
                        new Common.UserMessageForm("拖动的节点没有绑定相应的属性名").ShowDialog();
                        //new UserMessageForm().ShowDialog("拖动的节点没有绑定相应的属性名");
                        return;
                    }
                    if (DragNode.Name.Contains("RefSource"))
                    {
                        new Common.UserMessageForm("拖动的节点为非输出节点，请选择输出节点!").ShowDialog();
                        //new UserMessageForm().ShowDialog("拖动的节点为非输出节点，请选择输出节点!");
                        return;
                    }
                    if (!selectNode.Name.Contains("RefSource"))
                    {
                        new Common.UserMessageForm("接受的节点为非输入节点! 请选择正确的输入节点").ShowDialog();
                        //new UserMessageForm().ShowDialog("接受的节点为非输入节点! 请选择正确的输入节点");
                        return;
                    }
                    ...
                    if (DragNode.Tag == null)
                    {
                        new Common.UserMessageForm("拖动的节点或其父节点Tag属性没有绑定相应的对象").ShowDialog();
                        // new UserMessageForm().ShowDialog("拖动的节点或其父节点Tag属性没有绑定相应的对象");
                        return;
                    }
                    if (selectNode.Tag == null && selectNode.Parent.Tag == null) //selectNode.Tag == null &&  Parent.
                    {
                        new Common.UserMessageForm("接受的节点或其父节点Tag属性没有绑定相应的对象").ShowDialog();
                        //new UserMessageForm().ShowDialog("接受的节点或其父节点Tag属性没有绑定相应的对象");
                        return;
                    }
                    ////////////////////////////////////////////////
                        ...
                        case "RefSource1":
                            ...
                            if (DragNode.Parent == null)
                            {
                                if (((BaseFunction)selectNode.Tag).RefSource1.ContainsKey(DragNode.Name)) //Parent.
                                {
                                    ...
                                    {
                                        new Common.UserMessageForm("目标对象中已包含了一个相同的源对象").ShowDialog();
                                        //new UserMessageForm().ShowDialog("目标对象中已包含了一个相同的源对象");
                                        return;
                                    }
                                    else
        ...
        public void treeView1_Edite(object sender, TreeNode node, string treeViewName = "", ImageDataClass imageData = null)
        {
            if (node == null) return;
            //string name = node.Text;
            if (form != null)
            ...
            switch (node.Tag.GetType().Name)
            {
                ...
                case nameof(AcqSource):
                    acqSourceForm = new AcqSourceForm(AcqSourceManage.Instance.GetAcqSource(node.Text.Split('=')[1]));
                    acqSourceForm.Owner = this.parentForm;
                    acqSourceForm.TopMost = true;
                    break;
                ...
                case nameof(ReadObjectModel3D):
                    form = new ReadData3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(FaceToFaceDist3D):
                    form = new FaceToFaceDist3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(SampleObjectModel3D):
                    form = new SampleObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "选择对象3D":
                case nameof(SelectObjectModelRange3D):
                    form = new SelectionRangeObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "滤波对象3D":
                case nameof(FilterObjectModel3D):
                    form = new FilterObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "分割对象3D":
                case nameof(SegmentObjectModle3D):
                    form = new SegmentObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "合并对象3D":
                case nameof(Union3DobjectModel):
                    form = new UnionObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "平滑3D对象":
                case nameof(SmoothObjectModel3D):
                    form = new SmoothObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "平滑轮廓对象3D":
                case nameof(SmoothProfileModel3D):
                    form = new SmoothProfileObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "变换对象3D":
                case nameof(AffineObjectModel3D):
                    form = new AffineObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "缩放对象3D":
                case nameof(ScaleObjectModel3D):
                    form = new ScaleObjectModel3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "转换对象到图像3D":
                case nameof(TransformObject3DToXYZImage):
                    form = new TransformObject3DToImageForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "转换图像到对象3D":
                case nameof(TransformXYZImageToObject3D):
                    form = new TransformXYZImageToObject3DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "图像采集":
                case nameof(ImageAcq):
                    form = new ImageAcqForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "开始采集":
                case nameof(StartAcq):
                    form = new StartAcqForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "停止采集":
                case nameof(StopAcq):
                    form = new StopAcqForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "相机配置":
                case "传感器配置":
                case nameof(SensorConfig):
                    form = new SensorConfigForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "角平分线":
                case nameof(CalculateAngleBisector):
                    form = new CalculateAngleBisectorForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "中分线":
                case nameof(CalculateMiddleBisector):
                    form = new CalculateMiddleBisectorForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "点点距离":
                case nameof(PointToPointDist2D):
                    form = new PointToPointDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "线线距离":
                case nameof(LineToLineDist2D):
                    form = new LineToLineDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "点线距离":
                case nameof(PointToLineDist2D):
                    form = new PointToLineDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "圆线距离":
                case nameof(CircleToLineDist2D):
                    form = new CircleToLineDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "圆圆距离":
                case nameof(CircleToCircleDist2D):
                    form = new CircleToCircleDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "点到坐标系距离":
                case nameof(PointToCoordSysDist2D):
                    form = new PointToCoordSysDist2DForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "直线中点":
                case nameof(LineMiddlePoint):
                    form = new LineMiddlePointForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(NPointToLineDist2D):
                    form = new NPointToLineDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "轮廓到线距离":
                case nameof(ContToLineDist2D):
                    form = new ContToLineDist2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "Blob分析":
                case nameof(Blob):
                    form = new BlobForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "读取图像":
                case nameof(ReadImage):
                    form = new ReadImageForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case "轮廓变换":
                case "轨迹变换":
                case nameof(ContourAffine):
                    form = new ContourAffineForm(node); //
                    form.Owner = this.parentForm;
                    break;
                ...
                case "轮廓截剪":
                case nameof(ContourReduce):
                    form = new ContourReduceForm(node); // 
                    form.Owner = this.parentForm;
                    break;
                ...
                case "直线轮廓交点":
                case nameof(IntersectionLineContour):
                    form = new IntersectionLineContourForm((IFunction)node.Tag, node); //
                    form.Owner = this.parentForm;
                    break;
                ...
                case "多段线":
                case nameof(PolyLineMeasure):
                    form = new PolyLineMeasureForm((IFunction)node.Tag, node); // 
                    form.Owner = this.parentForm;
                    break;

                case "多边形":
                case nameof(PolygonMeasure):
                    form = new PolygonMeasureForm((IFunction)node.Tag, node); // 
                    form.Owner = this.parentForm;
                    break;
                ...
                case "手动取点":
                case nameof(ManualPointMeasure):
                    form = new ManualPointMeasureForm((IFunction)node.Tag, node); //
                    form.Owner = this.parentForm;
                    break;

                case "手动多段线":
                case nameof(ManualPolyLineMeasure):
                    form = new ManualPolyLineMeasureForm((IFunction)node.Tag, node); //
                    form.Owner = this.parentForm;
                    break;

                case "手动多边形":
                case nameof(ManualPolygonMeasure):
                    form = new ManualPolygonMeasureForm((IFunction)node.Tag, node); //
                    form.Owner = this.parentForm;
                    break;

                case "手动圆弧":
                case nameof(ManualCircleSectorMeasure):
                    form = new ManualCircleSectorMeasureForm((IFunction)node.Tag, node); //
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(FitCircle):
                    form = new FitCircleForm(node); //
                    form.Owner = this.parentForm;
                    break;

                case "拟合圆弧":
                case nameof(FitCircleSector):
                    form = new FitCircleSectorForm(node); //
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(FitEllipse):
                    form = new FitEllipseForm(node); //
                    form.Owner = this.parentForm;
                    break;

                //case "N点拟合椭圆":
                //case nameof(NPointsFitEllipse):
                //    form = new NPointFitEllipseForm( node); //
                //    form.Owner = this.parentForm;
                //    break;

                case "拟合矩形":
                case nameof(FitRect2):
                    form = new FitRect2Form(node); //
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(TileImage):
                    form = new TileImageForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "面面夹角":
                case nameof(FaceToFaceAngle):
                    form = new FaceToFaceAngleForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(AngleLineLine2D):
                    form = new AngleLineLine2DForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "数值计算":
                case nameof(NumericalCalculation):
                    form = new NumericalCalculationForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "极值点3D":
                case nameof(ExtremePoint3D):
                    form = new ExtremePointForm3D((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;

                case "点位运动":
                case nameof(PointMove):
                    form = new PointMoveForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "直线插补":
                case nameof(LineInterpolationMove):
                    form = new LineInterpolationMoveForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "螺旋线插补":
                case nameof(SpiralLineInterpolationMove):
                    form = new SpiralLineInterpolationMoveForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "矩形插补":
                case nameof(RectangleInterpolationMove):
                    form = new RectangleInterpolationMoveForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "轨迹运动":
                case nameof(TrackMove):
                    form = new TrackMoveForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "写入IO输出位控制":
                case nameof(WriteIoOutputBit):
                    form = new WriteIoOutputBitForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "写入IO输出组控制":
                case nameof(WriteIoOutputGroup):
                    form = new WriteIoOutputGroupForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "读取IO输入位控制":
                case nameof(ReadIoInputBit):
                    form = new ReadIoInputBitForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "读取IO输出位控制":
                case nameof(ReadIoOutputBit):
                    form = new ReadIoOutputBitForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;

                case "读取IO输出组控制":
                case nameof(ReadIoOutputGroup):
                    form = new ReadIoOutputGroupForm((IFunction)node.Tag);
                    form.TopMost = true;
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(ReadIoIntputGroup):
                    form = new ReadIoIntputGroupForm((IFunction)node.Tag);
                    form.Owner = this.parentForm;
                    break;
                ...
                case "轮廓偏置":
                case nameof(ContourOffset):
                    form = new ContourOffsetForm(node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(ProjectionPoint):
                    form = new ProjectionPointForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(LightConfig):
                    form = new LightConfigForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "直线插值":
                case nameof(LineInterpolation):
                    form = new LineInterpolationForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "圆弧插值":
                case nameof(CircleSectorInterpolation):
                    form = new CircleSectorInterpolationForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "数据输出":
                case nameof(OutputData):
                    form = new DataOutputForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "导圆角":
                case nameof(RoundCorners):
                    form = new RoundCornersForm(node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(ReferenceTeach):
                    form = new ReferenceTeachForm(node, imageData, treeViewName); //以视野的中心定为参考点
                    form.Owner = this.parentForm;
                ...
                case nameof(FeatureLocalization):
                    form = new FeatureLocalizationForm(node, imageData, treeViewName);
                    form.Owner = this.parentForm;
                    //if (treeViewName != null && treeViewName.Length > 0)
                    //    form.Text = treeViewName + "." + node.FullPath.Replace("\\", ".");
                    //else
                ...
                case nameof(MeasureArray):
                    form = new MeasureArrayForm(node, imageData, treeViewName);
                    form.Owner = this.parentForm;
                    //if (treeViewName != null && treeViewName.Length > 0)
                    //    form.Text = treeViewName + "." + node.FullPath.Replace("\\", ".");
                    //else
                ...
                case "流程单元":
                case nameof(JobUnit):
                    form = new JobUnitForm(node, imageData, treeViewName);
                    form.Owner = this.parentForm;
                    //if (treeViewName != null && treeViewName.Length > 0)
                    //    form.Text = treeViewName + "." + node.FullPath.Replace("\\", ".");
                    //else
                ...
                case nameof(Conditional):
                    form = new ConditionalForm(node, treeViewName);
                    form.Owner = this.parentForm;
                    break;  //
                case "条件语句<If>":
                case nameof(ConditionaIf):
                    form = new ConditionaIfForm(node, treeViewName);
                    form.Owner = this.parentForm;
                    break;  //
                case "条件语句<else>":
                case nameof(ConditionaElse):
                    form = new ConditionaElseForm(node, treeViewName);
                    form.Owner = this.parentForm;
                    break;  //

                case "Switch语句":
                case nameof(Switch):
                    form = new SwitchForm(node, treeViewName);
                    form.Owner = this.parentForm;
                    break;  //
                case "Case标签":
                case nameof(Case):
                    form = new CaseForm(node, treeViewName);
                    form.Owner = this.parentForm;
                    break;  //
                ...
                //case "迁移计算":
                //case nameof(MigrateCalculate):
                //    form = new MigrateCalculateForm(node);
                //    form.Owner = this.parentForm;
                //    break;  //
                ...
                case "向量对位":
                case nameof(VectorAlign):
                    form = new VectorAlignForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "纠偏计算":
                case nameof(RectifyCalculate):
                    form = new RectifyCalculateForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "向量对位2":
                case nameof(VectorAlignNew):
                    form = new VectorAlignNewForm(node);
                    form.Owner = this.parentForm;
                    break;

                case "向量计算":
                case nameof(VectorCalculate):
                    form = new VectorCalculateForm(node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case nameof(NccModelMatch):
                    form = new NccMatchForm(node);
                    form.Owner = this.parentForm;
                    break;  //

                case "轮廓匹配":
                case nameof(ContourModelMatch2D):
                    form = new ContourModelMatch2DForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                ...
                case nameof(ShapeModelMatch2D):
                    form = new ShapeModelMatch2DForm(node); //
                    form.Owner = this.parentForm;
                    break;
                ...
                case "裁剪图像2":
                case "图像裁剪2":
                case nameof(ImageReduceROI):
                    form = new ImageReduceROIForm(node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case "图像分解":
                case nameof(ImageDecompose):
                    form = new ImageDecomposeForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break; // SendHWindowImage

                case "图像校正":
                case nameof(ImageRectify):
                    form = new ImageRectifyForm((IFunction)node.Tag, node);
                    form.Owner = this.parentForm;
                    break; // 

                case "发送图像":
                case nameof(SendHWindowImage):
                    form = new SendHWindowImageForm(node);
                    form.Owner = this.parentForm;
                    break; // 
                ...
                case "发送数据RFID":
                case nameof(SendDataRFID):
                    form = new SendDataRFIDForm(node);
                    form.Owner = this.parentForm;
                    break; // 
                ...
                case "读取坐标点":
                case nameof(ReadCoordPoint):
                    form = new ReadCoordPointForm(node);
                    form.Owner = this.parentForm;
                    break; // 等待数据

                case "读取对象":
                case nameof(ReadObject):
                    form = new ReadObjectForm(node);
                    form.Owner = this.parentForm;
                    break; // 等待数据
                ...
                case "条件判断":
                case nameof(ConditionalJudge):
                    form = new ConditionalJudgeForm(node);
                    form.Owner = this.parentForm;
                    break;
                ...
                case "鼠标点击":
                case nameof(MousesClick):
                    form = new MouseClickForm(node);
                    form.Owner = this.parentForm;
                    break;  //  

                case "设置光标":
                case nameof(SetCursor):
                    form = new SetCursorForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "读取文本":
                case nameof(ReadText):
                    form = new ReadTextForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "等待文本":
                case nameof(WaitText):
                    form = new WaitTextForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "写入文本":
                case nameof(WriteText):
                    form = new WriteTextForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "键盘点击":
                case nameof(KeyBoardClick):
                    form = new KeyBoardClickForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "坐标转换":
                case nameof(CoordTransform):
                    form = new CoordTransformForm(node);
                    form.Owner = this.parentForm;
                    break;  // 

                case "Wafer寻晶":
                case nameof(WaferFindDie):
                    form = new WaferFindDieForm(node);
                    form.Owner = this.parentForm;
                    break;  // 
                ...
                //    form = new DefectDetectingForm(node, imageData);
                //    form.Owner = this.parentForm;
                ...
                case "宽度检测":
                case nameof(WidthDetect):
                    form = new WidthDetectFormNew(node);
                    form.Owner = this.parentForm;
                    break; // 

                case "变化模型":
                case nameof(VariationModeDetect):
                    form = new VariationModelForm(node);
                    form.Owner = this.parentForm;
                    break; // 

                case "缺陷检测":
                case "瑕疵检测":
                case nameof(FlawDetect):
                    form = new FlawDetectForm(node);
                    form.Owner = this.parentForm;
                    break; // 
                ...
                case "上料穴位":
                case "取料计算":
                case nameof(RobotGrab):
                    form = new RobotGrabForm(node);
                    form.Owner = this.parentForm;
                    break; // 
                #endregion
                ...
                #region 点胶功能
                case "轨迹导出":
                case nameof(ContourExport):
                    form = new ContourExportForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                case "轨迹读取":
                case nameof(ContourRead):
                    form = new ContourReadForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                case "轨迹堆叠":
                case nameof(TrackStack):
                    form = new TrackStackForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                case "轨迹合并":
                case nameof(TrackCompose):
                    form = new TrackComposeForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                case "轨迹捨取":
                case nameof(ContourPick):
                    form = new ContourPickForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                case "轨迹转换":
                case nameof(TrackTrans):
                    form = new TrackTransForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                case "轨迹自检":
                case nameof(TrackSelfCheck):
                    form = new TrackSelfCheckForm(node);
                    form.Owner = this.parentForm;
                    break;  //
                //case "轨迹变换":
                ...
                case "轨迹提取":
                case nameof(ContourExtract):
                    form = new ContourExtractForm(node);
                    form.Owner = this.parentForm;
                    break; //  

                case "轨迹偏置":
                case nameof(TrackOffset):
                    form = new TrackOffsetForm(node);
                    form.Owner = this.parentForm;
                    break; //

                case "轮廓发送":
                case nameof(ContourSend):
                    form = new ContourSendForm(node);
                    form.Owner = this.parentForm;
                    break; //
                ...
                #region
                case "点补偿":
                case nameof(PointCompensate):
                    form = new PointCompensateForm(node);
                    form.Owner = this.parentForm;
                    break;  // 
                #endregion
