...
{
    public partial class GlueAlignGuidedForm : Form
    {
        ...
        public GlueAlignGuidedForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public GlueAlignGuidedForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void BindProperty()
        {
            try
            {
                ...
                //////////////////////////////////////////////////
                this.数据写入dataGridView.DataSource = ((GlueAlignGuided)this._function).DataList;
            }
            catch (Exception ee)
            {
        ...
        private void GlueAlignGuidedForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (cts != null)
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<WriteCommunicateCommand > WriteDataList = ((BindingList<WriteCommunicateCommand >)((GlueAlignGuided)this._function).DataList);
