...
{
    public partial class CoordMapForm : Form
    {
        ...
        public CoordMapForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
        }

        public CoordMapForm(TreeNode node)
        {
            this._refNode = node;
        ...
        private void CoordMapForm_Load(object sender, EventArgs e)
        {
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                CoordPoint coordPoint1 = ((CoordMap)this._function).CoordPoint1;
                CoordPoint coordPoint2 = ((CoordMap)this._function).CoordPoint2;
                if (coordPoint1 != null)
                {
                    for (int i = 0; i < coordPoint1.X.Length; i++)
                    {
                        this.P坐标dataGridView.Rows.Add(coordPoint1.X[i], coordPoint1.Y[i]);
                    }
                }
                if (coordPoint2 != null)
                {
                    for (int i = 0; i < coordPoint2.X.Length; i++)
                    {
                        this.Q坐标dataGridView.Rows.Add(coordPoint2.X[i], coordPoint2.Y[i]);
                    }
                }
            }
            catch (Exception ee)
