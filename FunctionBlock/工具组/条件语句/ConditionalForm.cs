...
namespace FunctionBlock
{
    public partial class ConditionalForm : Form
    {
        ...
        private Conditional _ForLoopControl;
        ...
        public ConditionalForm(IFunction function)
        {
        ...
        public ConditionalForm(TreeNode node, string treeViewName = null)
        {
        ...
        private void ConcurrentExecutionForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                ((Conditional)_refNode.Tag).ParentNode = _refNode;
        ...
        private void 数据写入dataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    BindingList<JudgeCommand> WriteDataList = ((BindingList<JudgeCommand>)((Conditional)this._function).DataList);
                    switch (数据写入dataGridView.Columns[e.ColumnIndex].Name)
                    {
                        case "DeleteBtn": //InsertBtn
                            ...
                            break;
                        case "InsertBtn": //
                            WriteDataList.Insert(e.RowIndex, new JudgeCommand());
                            break;
                        case "UpMoveCol":
                            if (e.RowIndex > 0)
                            {
                                JudgeCommand up = WriteDataList[e.RowIndex - 1];
                                JudgeCommand cur = WriteDataList[e.RowIndex];
                        ...
                        case "DownMoveCol":
                            ...
                            {
                                JudgeCommand down = WriteDataList[e.RowIndex + 1];
                                JudgeCommand cur = WriteDataList[e.RowIndex];
                                WriteDataList[e.RowIndex + 1] = cur;
        ...
        {
            ConditionalForm_MouseDown(null, null);
        }
        ...
        private void ConditionalForm_MouseDown(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
