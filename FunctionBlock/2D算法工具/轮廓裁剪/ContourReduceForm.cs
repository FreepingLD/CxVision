...
{
    public partial class ContourReduceForm : Form
    {
        private Form form;
        ...
        public ContourReduceForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        private void ContourReduceForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.dataGridView1.DataSource = ((ContourReduce)this._function).Param;
        ...
        private void dataGridViewContextMenuStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            try
            {
                BindingList<ReduceContourParam> listShape = ((ContourReduce)this._function).Param;
                ...
                switch (name)
                {
                    case "复制":
                        ...
                        if (copyForm.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            {
                                ReduceContourParam roi = ((ContourReduce)this._function).Param[index];
                                ReduceContourParam roiParam = new ReduceContourParam();
                                roiParam.ShapeType = roi.ShapeType;
                                roiParam.RoiShape = roi.RoiShape.Clone();
                                ((ContourReduce)this._function).Param.Add(roiParam);
                            }
                        }
                        break;
                    case "设置形状参数":
                        ...
                        switch (listShape[index].ShapeType)
                        {
                            case enShapeType.矩形1:
                                WcsRet1ParamForm ret1Param = new WcsRet1ParamForm(listShape[index].RoiShape as drawWcsRect1);
                                if (ret1Param.ShowDialog() == DialogResult.OK)
                                {
                                    drawWcsRect1 wcsRect1 = listShape[index].RoiShape as drawWcsRect1;
                                    drawWcsRect1 wcsRect1New = ret1Param.WcsRect1;
                                    wcsRect1.X1 = wcsRect1New.X1;
                                    wcsRect1.Y1 = wcsRect1New.Y1;
                                    wcsRect1.X2 = wcsRect1New.X2;
                                    wcsRect1.Y2 = wcsRect1New.Y2;
                                    /////////////////////////////////////////////
                                    userWcsCoordSystem wcsCoordSystem = ((ContourReduce)this._function).WcsCoordSystem;
                                    this.drawObject.ClearViewObject();
                                    this.drawObject.AddViewObject(new ViewData(listShape[index].RoiShape.AffineWcsROI(wcsCoordSystem?.GetHomMat2D()).GetXLD(this.drawObject.CameraParam), enColor.orange.ToString()));
                                }
                                break;
        ...
        private void DisplayObjectModel(object sender, ExcuteCompletedEventArgs e)  // 这里一定要加一个键值对变量来收集，以免出现重合
        {
            ...
            {
                new Common.UserMessageForm("DisplayObjectModel->操作失败").ShowDialog();
        ...
        private void ContourReduceForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        private void 显示条目comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (this.显示条目comboBox.SelectedIndex == -1) return; //TreeNode
                ((ContourReduce)this._function).SetPropertyValues("TreeNode", this._refNode);
                switch (this.显示条目comboBox.SelectedItem.ToString())
                {
                    case "输入轮廓":
                        userWcsPolyLine wcsPolyLine = ((ContourReduce)this._function).WcsPolyLine;
                        if (wcsPolyLine != null)
                        {
                            ...
                            this.drawObject.PointCloudModel3D = new PointCloudData(hObjectModel3D);
                        }
                        break;
                    case "载前轮廓":
                    case "输出轮廓":
                        wcsPolyLine = ((ContourReduce)this._function).ReduceContour;
                        if (wcsPolyLine != null)
                        {
                            double[] x = new double[wcsPolyLine.X.Count];
                            double[] y = new double[wcsPolyLine.X.Count];
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    int index = 0;
                    WcsROI wcsShape;
                    if (((ContourReduce)this._function).Param == null)
                        ((ContourReduce)this._function).Param = new BindingList<ReduceContourParam>();
                    BindingList<ReduceContourParam> listShape = ((ContourReduce)this._function).Param;
                    userWcsCoordSystem wcsCoordSystem = ((ContourReduce)this._function).WcsCoordSystem;
                    switch (dataGridView1.Columns[e.ColumnIndex].Name)
                    {
                        case "TeachCol":
                            if (this.dataGridView1.Rows[e.RowIndex].DataBoundItem == null)
                            {
                                ...
                                //new UserMessageForm().ShowDialog();
                                return;
                            }
                            ...
                            foreach (var item in listShape)
                            {
                                ...
                                {
                                    this.drawObject.AddViewObject(new ViewData(item.RoiShape.AffineWcsROI(wcsCoordSystem?.GetHomMat2D()).GetXLD(this.drawObject.CameraParam), enColor.orange.ToString()));
                                }
                                index++;
                            }
                            if (this.drawObject.PointCloudModel3D == null)
                                this.drawObject.PointCloudModel3D = new PointCloudData(((ContourReduce)this._function).WcsPolyLine.GetHObjectModel3D());
                            //////////////////////////////////////////////////////////////////////////////
                            ...
                            else
                            {
                                ...
                                this.drawObject.SetParam(listShape[e.RowIndex].RoiShape.AffineWcsROI(wcsCoordSystem?.GetHomMat2D()));
                            }
                            this.drawObject.DrawWcsRoiShapeOnWindow(enColor.red, out wcsShape);
                            this.drawObject.AddViewObject(new ViewData(wcsShape.AffineWcsROI(wcsCoordSystem?.GetHomMat2D()).GetXLD(this.drawObject.CameraParam), enColor.orange.ToString())); //这里也要做下变换
                            /////////////////////////////////////////////////////////
                        ...
                        default:
                            ...
                            foreach (var item in listShape)
                            {
                                ...
                                if (index == e.RowIndex)
                                {
                                    this.drawObject.AddViewObject(new ViewData(item.RoiShape.AffineWcsROI(wcsCoordSystem?.GetHomMat2D()).GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                                }
                                else
                                {
                                    this.drawObject.AddViewObject(new ViewData(item.RoiShape.AffineWcsROI(wcsCoordSystem?.GetHomMat2D()).GetXLD(this.drawObject.CameraParam), enColor.orange.ToString()));
