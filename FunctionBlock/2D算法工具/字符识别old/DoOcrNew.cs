...
namespace FunctionBlock
{
    ...
    public class DoOcrNew
    {
        ...
        public void DoOcrDetect(HImage hImage)
        {
            HImage invertImage;
        ...
        public void DoOcrDetect(HImage hImage, HRegion chartRegion)
        {
            if (hImage == null || !hImage.IsInitialized())
                throw new ArgumentNullException();
            if (chartRegion == null || !chartRegion.IsInitialized())
                throw new ArgumentNullException();
            /////////////////////////////////////////////////////////
            HImage invertImage;
            if (this.OcrParam.InvertImage)
            ...
            {
                case enOcrModel.OcrMLP:
                    OcrResult = DoOcrMlp(invertImage, chartRegion, (OcrMlpParam)OcrParam);
                    break;
                case enOcrModel.OcrCNN:
                    OcrResult = DoOcrCnn(invertImage, chartRegion, (OcrCnnParam)OcrParam);
                    break;
                case enOcrModel.OcrKNN:
                    OcrResult = DoOcrKnn(invertImage, chartRegion, (OcrKnnParam)OcrParam);
                    break;
                case enOcrModel.OcrSVM:
                    OcrResult = DoOcrSvm(invertImage, chartRegion, (OcrSvmParam)OcrParam);
                    break;
                case enOcrModel.OcrTextMode:
                    OcrResult = DoOcrTextModel(invertImage, chartRegion, (OcrTextModelParam)OcrParam);
                    break;
            }
        }

        private OcrResult DoOcrCnn(HImage hImage, HRegion chartRegion, OcrCnnParam ocrParam)
        {
            OcrResult ocrResult = new OcrResult();
            HTuple confidence, Class;
            string Word;
            double Score;
            HRegion selectRegion = null, unionRegion = null;
            ...
            try
            {
                switch (ocrParam.OcrMethod)
                {
                    ...
                    case enOcrMethod.MultipleCharacters:
                        /////////
                        Class = oCRCnn.DoOcrMultiClassCnn(chartRegion, hImage, out confidence);
                    ...
                    case enOcrMethod.SingleCharacter:
                        int count = chartRegion.CountObj();
                        for (int i = 1; i <= count; i++)
                        {
                            selectRegion = chartRegion.SelectObj(i);
                            ///////////////////////////////////////
                            if (selectRegion.Area > 0)
                            {
                    ...
                    case enOcrMethod.Word:
                        /////////////////////////////////////
                        Class = oCRCnn.DoOcrWordCnn(chartRegion, hImage, ocrParam.Expression, ocrParam.NumAlternatives, ocrParam.NumCorrections, out confidence, out Word, out Score);
        ...
        private OcrResult DoOcrKnn(HImage hImage, HRegion chartRegion, OcrKnnParam ocrParam)
        {
            OcrResult ocrResult = new OcrResult();
            HTuple confidence, Class;
            string Word;
            ...
            try
            {
                switch (ocrParam.OcrMethod)
                {
                    ...
                    case enOcrMethod.MultipleCharacters:
                        /////////
                        Class = oCRKnn.DoOcrMultiClassKnn(chartRegion, hImage, out confidence);
                        foreach (var item in Class.SArr)
                        {
                            ocrResult.Character += item;
                        }
                    ...
                    case enOcrMethod.Word:
                        /////////////////////////////////////
                        Class = oCRKnn.DoOcrWordKnn(chartRegion, hImage, ocrParam.Expression, ocrParam.NumAlternatives, ocrParam.NumCorrections, out confidence, out Word, out Score);
                        foreach (var item1 in Class.SArr)
                        {
                            ocrResult.Character += item1;
                        }
        ...
        private OcrResult DoOcrSvm(HImage hImage, HRegion chartRegion, OcrSvmParam ocrParam)
        {
            OcrResult ocrResult = new OcrResult();
            HTuple confidence, Class;
            string Word;
            ...
            try
            {
                switch (ocrParam.OcrMethod)
                {
                    ...
                    case enOcrMethod.MultipleCharacters:
                        /////////
                        Class = oCRSvm.DoOcrMultiClassSvm(chartRegion, hImage);
                        foreach (var item1 in Class.SArr)
                        {
                            ocrResult.Character += item1;
                        }
                    ...
                    case enOcrMethod.Word:
                        /////////////////////////////////////
                        Class = oCRSvm.DoOcrWordSvm(chartRegion, hImage, ocrParam.Expression, ocrParam.NumAlternatives, ocrParam.NumCorrections, out Word, out Score);
                        foreach (var item1 in Class.SArr)
                        {
                            ocrResult.Character += item1;
                        }
        ...
        private OcrResult DoOcrMlp(HImage hImage, HRegion chartRegion, OcrMlpParam ocrParam)
        {
            OcrResult ocrResult = new OcrResult();
            HTuple confidence, Class;
            string Word;
            ...
            try
            {
                switch (ocrParam.OcrMethod)
                {
                    default:
                    case enOcrMethod.MultipleCharacters:
                        Class = oCRMlp.DoOcrMultiClassMlp(chartRegion, hImage, out confidence);
                        foreach (var item in Class.SArr)
                        {
                            ocrResult.Character += item;
                        }
                    ...
                    case enOcrMethod.Word:
                        /////////////////////////////////////
                        Class = oCRMlp.DoOcrWordMlp(chartRegion, hImage, ocrParam.Expression, ocrParam.NumAlternatives, ocrParam.NumCorrections, out confidence, out Word, out Score);
                        foreach (var item1 in Class.SArr)
                        {
                            ocrResult.Character += item1;
                        }
        ...
        private OcrResult DoOcrTextModel(HImage hImage, HRegion chartRegion, OcrTextModelParam ocrParam)
        {
            OcrResult ocrResult = new OcrResult();
            ...
            try
            {
                ...
                hTextResult = oCRTextModel.FindText(hImage);
