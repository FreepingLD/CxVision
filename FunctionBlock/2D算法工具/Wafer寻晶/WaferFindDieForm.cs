...
{
    public partial class WaferFindDieForm : Form
    {
        protected Form form;
        protected IFunction _function;
        private VisualizeView drawObject;
        //private RunToolStripWrapClass runToolStrip;
        public WaferFindDieForm(TreeNode node)
        {
            InitializeComponent();
        ...
        public void WaferFindDieForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        protected void BindProperty()
        {
            try
            {
                ...
                this.起始点位置comboBox.DataSource = Enum.GetNames(typeof(enCoordQuadrant));
                this.坐标系名称comboBox.DataSource = Enum.GetNames(typeof(enCoordSysName));
                this.寻晶算法comboBox.DataSource = Enum.GetNames(typeof(enWaferFindDie));
                this.参考点comboBox.DataSource = Enum.GetNames(typeof(enRefObject));
                WaferFindDieParam param = ((WaferFindDie)this._function).Param ;
                this.寻晶算法comboBox.DataBindings.Add(nameof(this.寻晶算法comboBox.Text), param, nameof(param.WaferFindDie), true, DataSourceUpdateMode.OnPropertyChanged);
                this.间距_XcomboBox.DataBindings.Add(nameof(this.间距_XcomboBox.Text), param, nameof(param.Dist_X), true, DataSourceUpdateMode.OnPropertyChanged);
                this.间距_YcomboBox.DataBindings.Add(nameof(this.间距_YcomboBox.Text), param, nameof(param.Dist_Y), true, DataSourceUpdateMode.OnPropertyChanged);
                this.起始点位置comboBox.DataBindings.Add(nameof(this.起始点位置comboBox.Text), param, nameof(param.CoordQuadrant), true, DataSourceUpdateMode.OnPropertyChanged);
                this.坐标系名称comboBox.DataBindings.Add(nameof(this.坐标系名称comboBox.Text), param, nameof(param.CoordSysName), true, DataSourceUpdateMode.OnPropertyChanged);
                this.参考点comboBox.DataBindings.Add(nameof(this.参考点comboBox.Text), param, nameof(param.RefObject), true, DataSourceUpdateMode.OnPropertyChanged);
                this.角度公差comboBox.DataBindings.Add(nameof(this.角度公差comboBox.Text), param, nameof(param.AngleTolerance), true, DataSourceUpdateMode.OnPropertyChanged);
                this.输出匹配点checkBox.DataBindings.Add(nameof(this.输出匹配点checkBox.Checked), param, nameof(param.IsOutputMatchPoint), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (object3D != null)
                {
                    switch (object3D.GetType().Name)
                    {
                        ...
                        case "userWcsPoint":
                            this.drawObject.AttachPropertyData.Clear();
                            userWcsPoint wcsPoint5 = (userWcsPoint)object3D;
                            this.drawObject.AttachPropertyData.Add(wcsPoint5);
                            break;
                    }
                }
            }
        ...
        protected void WaferFindDieForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
