...
{
    public partial class FitEllipseSectorForm : Form
    {
        protected Form form;
        protected IFunction _function;
        private VisualizeView drawObject;
        private TreeNode _refNode;
        public FitEllipseSectorForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        public void FitEllipseSectorForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        public enum enShowItemsEllipseSector
        {
            输入对象,
            输出对象,
        }
        protected void BindProperty()
        {
            ...
            {
                // 创建匹配参数
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(enShowItemsEllipseSector));
                //this.结果dataGridView.DataSource = ((FunctionBlock.FitCircle)this._function).ResultDataTable;
                EllipseFitParam param = ((FitEllipseSector)this._function).FitParam;
                ...
                this.轮廓数量comboBox.DataBindings.Add(nameof(this.轮廓数量comboBox.Text), param, nameof(param.PointCount), true, DataSourceUpdateMode.OnPropertyChanged);
                this.圆弧方向comboBox.DataBindings.Add(nameof(this.圆弧方向comboBox.Text), param, nameof(param.PointOrder), true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ex)
            {
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case nameof(enShowItemsEllipseSector.输入对象):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsPoint[] wcsPoints = ((FitEllipseSector)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                        ...
                        break;
                    case nameof(enShowItemsEllipseSector.输出对象):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsEllipseSector circleSector = ((FitEllipseSector)this._function).WcsEllipseSector;
                        if (circleSector != null && circleSector.EdgesPoint_xyz != null)
                        {
                            double[] x = new double[circleSector.EdgesPoint_xyz.Length];
                            double[] y = new double[circleSector.EdgesPoint_xyz.Length];
                            double[] z = new double[circleSector.EdgesPoint_xyz.Length];
                            for (int i = 0; i < circleSector.EdgesPoint_xyz.Length; i++)
                            {
                                x[i] = circleSector.EdgesPoint_xyz[i].X;
                                y[i] = circleSector.EdgesPoint_xyz[i].Y;
                                z[i] = circleSector.EdgesPoint_xyz[i].Z;
                            }
                            HObjectModel3D hObjectModel3D = new HObjectModel3D(x, y, z);
                            this.drawObject.PointCloudModel3D = new PointCloudData(hObjectModel3D);
                        }
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.AddViewObject(new ViewData(((FitEllipseSector)this._function).WcsEllipseSector.GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                                }));
                            }
                            else
                            {
        ...
        protected void FitEllipseSectorForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
