...
namespace FunctionBlock
{
    public partial class FitCircleForm : Form
    {
        ...
        public FitCircleForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
            ...
            try
            {
                switch (this.显示条目comboBox.Text.Trim())
                {
                    case nameof(enShowItems.输入对象):
                        ...
                        userWcsPoint[] wcsPoints = ((FitCircle)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                    ...
                    case nameof(enShowItems.输出对象):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsCircle circleSector = ((FitCircle)this._function).WcsCircle;
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.AddViewObject(new ViewData(((FitCircle)this._function).WcsCircle.GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                                }));
                            }
                            else
                            {
