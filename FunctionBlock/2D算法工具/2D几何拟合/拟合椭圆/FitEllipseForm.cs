...
{
    public partial class FitEllipseForm : Form
    {
        protected Form form;
        protected IFunction _function;
        private VisualizeView drawObject;
        private TreeNode _refNode;
        public FitEllipseForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        public void FitCircleSectorForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        public enum enShowItemsEllipse
        {
            输入对象,
            输出对象,
        }
        protected void BindProperty()
        {
            ...
            {
                // 创建匹配参数
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(enShowItemsEllipse));
                //this.结果dataGridView.DataSource = ((FunctionBlock.FitCircle)this._function).ResultDataTable;
                EllipseFitParam param = ((FitEllipse)this._function).FitParam;
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case nameof(enShowItemsEllipse.输入对象):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsPoint[] wcsPoints = ((FitEllipse)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                        ...
                        break;
                    case nameof(enShowItemsEllipse.输出对象):
                        this.drawObject.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsEllipse circleSector = ((FitEllipse)this._function).WcsEllipse;
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.AddViewObject(new ViewData(((FitEllipse)this._function).WcsEllipse.GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                                }));
                            }
                            else
                            {
        ...
        protected void FitEllipseForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
