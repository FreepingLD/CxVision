...
{
    public partial class FitLineForm : Form
    {
        protected Form form;
        ...
        public FitLineForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
            this._function = this._refNode as IFunction;
        ...
        protected void BindProperty()
        {
            ...
            {
                // 创建匹配参数
                this.显示条目comboBox.DataSource = Enum.GetNames(typeof(enShowLineItem));
                LineFitParam param = ((FitLine)this._function).FitParam;
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                ((FitLine)this._function).SetPropertyValues("TreeNode", this._refNode);
                ///////////////////////////////////////////
                switch (this.显示条目comboBox.Text.Trim())
                {
                    case nameof(enShowLineItem.输入点):
                        userWcsPoint [] wcsPoints  = ((FitLine)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                        ...
                        break;
                    case nameof(enShowLineItem.输出直线):
                        userWcsLine wcsLine  = ((FitLine)this._function).WcsLine;
                        this.drawObject.AddViewObject(new ViewData(wcsLine.GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                        break;
                }
            }
            catch (Exception ex)
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.AddViewObject(new ViewData(((FitLine)this._function).WcsLine.GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                                }));
                            }
                            else
                            {
        ...
        protected void FitLineForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // 注消事件
            try
            {
