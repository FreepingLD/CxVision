...
{
    public partial class FitCircleSectorForm : Form
    {
        protected Form form;
        protected IFunction _function;
        private VisualizeView drawObject;
        private TreeNode _refNode;
        public FitCircleSectorForm(TreeNode node)
        {
            InitializeComponent();
            this._refNode = node;
        ...
        protected void BindProperty()
        {
            try
            {
                ...
                CircleFitParam param = ((FitCircleSector)this._function).FitParam;
        ...
        private void 显示条目comboBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case nameof(enShowItems.输入对象):
                        this.drawObject?.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsPoint[] wcsPoints = ((FitCircleSector)this._function).WcsPoint;
                        if (wcsPoints != null)
                        {
                    ...
                    case nameof(enShowItems.输出对象):
                        this.drawObject?.PointCloudModel3D?.ClearObjectModel3d();
                        userWcsCircleSector circleSector = ((FitCircleSector)this._function).WcsCircleSector;
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.AddViewObject(new ViewData(((FitCircleSector)this._function).WcsCircleSector.GetXLD(this.drawObject.CameraParam), enColor.green.ToString()));
                                }));
                            }
                            else
                            {
