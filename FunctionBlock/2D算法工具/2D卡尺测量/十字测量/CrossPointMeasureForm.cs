...
{
    public partial class CrossPointMeasureForm : Form
    {
        private object _objectDataModel;
        private Form form;
        private IFunction _function;
        private userDrawCrossMeasure drawObject;
        private TreeNode _refNode;
        private MetrolegyParamForm metrolegyParam;
        public CrossPointMeasureForm(IFunction function,TreeNode node)
        {
        ...
        private void CrossPointMeasureForm_Load(object sender, EventArgs e)
        {
            ...
            //////////////////////////////////////
            this.drawObject.BackImage = ((FunctionBlock.CrossPointMeasure)_function).ImageData;
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.CrossPointMeasure)this._function), "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixLineParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.CrossPointMeasure)this._function).WcsPoint.GetPixPoint().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((CrossPointMeasure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((CrossPointMeasure)this._refNode.Tag).FindCrossPoint.LinePixPosition.AffinePixLine2D(((CrossPointMeasure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((CrossPointMeasure)this._refNode.Tag).ImageData != null ? ((CrossPointMeasure)this._refNode.Tag).ImageData : null;
