...
{
    public partial class ManualPolygonMeasureForm : Form
    {
        private object _objectDataModel;
        private Form form;
        private IFunction _function;
        private userDrawManualPointMeasure drawObject;
        private TreeNode _refNode;
        private MetrolegyParamForm metrolegyParam;
        public ManualPolygonMeasureForm(IFunction function,TreeNode node)
        {
            this._function = function;
            this._refNode = node;
            InitializeComponent();
            this.Text = function.GetPropertyValues("名称").ToString();
            this.drawObject = new userDrawManualPointMeasure(this.hWindowControl1, ((FunctionBlock.ManualPointMeasure)_function).FindCrossPoint.PointPixPosition.AffineTransPixPoint(((ManualPointMeasure)_function).PixCoordSystem?.GetVariationHomMat2D()), ((FunctionBlock.ManualPointMeasure)_function).PixCoordSystem);
            new ListBoxWrapClass().InitListBox(this.listBox1, node);
            new ListBoxWrapClass().InitListBox(this.listBox2, node, 2);
        }
        private void ManualPolygonMeasureForm_Load(object sender, EventArgs e)
        {
            ...
            //////////////////////////////////////
            this.drawObject.BackImage = ((FunctionBlock.ManualPolygonMeasure)_function).ImageData;
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.ManualPolygonMeasure)this._function), "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////////////
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixPolygonParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.ManualPolygonMeasure)this._function).WcsPolygon.GetPixPolygon().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((ManualPolygonMeasure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((ManualPolygonMeasure)this._refNode.Tag).FindPolygon.PolygonPixPosition.AffinePixPolygon(((ManualPolygonMeasure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((ManualPolygonMeasure)this._refNode.Tag).ImageData != null ? ((ManualPolygonMeasure)this._refNode.Tag).ImageData : null;
        ...
        private void ManualPolygonMeasureForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
