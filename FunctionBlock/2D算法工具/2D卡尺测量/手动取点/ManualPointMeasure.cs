...
namespace FunctionBlock
{
    ...
    public class ManualPointMeasure : BaseFunction, IFunction
    {
        [NonSerialized]
        private ImageDataClass _imageData;
        private FindManualPoint _findCrossPoint = new FindManualPoint();
        ...
        public FindManualPoint FindCrossPoint { get => _findCrossPoint; set => _findCrossPoint = value; }
        ...
        public ManualPointMeasure(string acqSourceName, IFunction coordSystem)
        {
        ...
        public ManualPointMeasure(IFunction imageSource, IFunction coordSystem)
        {
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            try
            {
                    ...
                    foreach (var item in param)
                    {
                        ...
                        switch (item.GetType().Name)
                        {
                            case nameof(userWcsPoint):
                                this._findCrossPoint.PointWcsPosition = (userWcsPoint)item;
                                break;
                            case nameof(userPixPoint):
                                this._findCrossPoint.PointPixPosition = (userPixPoint)item;
                                break;
                            case nameof(userPixCoordSystem):
                ...
                userWcsPoint camPoint = this._findCrossPoint.FitPoint.GetWcsCamPoint();
                stopwatch.Stop();         
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[0].SetValue(this.name, "X", this._findCrossPoint.FitPoint.X);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[1].SetValue(this.name, "Y", this._findCrossPoint.FitPoint.Y);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[2].SetValue(this.name, "Z", this._findCrossPoint.FitPoint.Z);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[3].SetValue(this.name, "Cam_X", camPoint.X);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[4].SetValue(this.name, "Cam_Y", camPoint.Y);
                ///////////////////////////////////////////////
                ...
                /////////
                LoggerHelper.Info(this.name + ":" + this._findCrossPoint.FitPoint.ToString(), this._imageData?.CamName);
                ...
                OnExcuteCompleted(this._imageData?.CamName, this._imageData?.ViewWindow, this.name, this._findCrossPoint.FitPoint);
            }
            catch (Exception ex)
            {
                this.Result.Succss = false;
                LoggerHelper.Error(this.name + "执行报错" + ex, this._imageData?.CamName);
            }
            finally
            {
            ...
            if (this.Result.Succss)
                LoggerHelper.Info(this.name + "->执行成功", this._imageData?.CamName);
            else
                LoggerHelper.Error(this.name + "->执行失败", this._imageData?.CamName);
        ...
        public object GetPropertyValues(string propertyName)
        {
            switch (propertyName)
            {
                ...
                case nameof(this.FindCrossPoint):
                    return this._findCrossPoint.FitPoint;
            }
        }
