...
namespace FunctionBlock
{
    public partial class PolyLineMeasureForm : Form
    {
        ...
        public PolyLineMeasureForm(IFunction function, TreeNode node)
        {
            this._refNode = node;
            this._function = function;
            InitializeComponent();
        ...
        {
            ///////////////
            this.drawObject.BackImage = ((PolyLineMeasure)_function).ImageData;
            BindProperty();
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.PolyLineMeasure)this._function), "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                ///////////////////
            }
            catch (Exception ex)
            {
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixPolyLineParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow(); 
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.PolyLineMeasure)this._function).WcsPolyLine.GetPixPolyLine().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((PolyLineMeasure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((PolyLineMeasure)this._refNode.Tag).FindPolyLine.PolyLinePixPosition.AffinePixPolyLine(((PolyLineMeasure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((PolyLineMeasure)this._refNode.Tag).ImageData != null ? ((PolyLineMeasure)this._refNode.Tag).ImageData : null;
