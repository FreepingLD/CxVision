...
namespace FunctionBlock
{
    public partial class EllipseMeasureForm : Form
    {
            ...
            try
            {
                switch (e.ClickedItem.Name)
                {
                    ...
                    case "执行":
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixEllipseParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.EllipseMeasure)this._function).WcsEllipse.GetPixEllipse().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        private void FindLineForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                new Common.UserMessageForm("更新点位报错").ShowDialog();
                //new UserMessageForm().ShowDialog("更新点位报错");
            }
        }
