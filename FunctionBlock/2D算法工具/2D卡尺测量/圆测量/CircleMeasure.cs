...
namespace FunctionBlock
{
    ...
    public class CircleMeasure : BaseFunction, IFunction
    {
        [NonSerialized]
        ...
        private FindCircle _findCircle = new FindCircle();
        ...
        public userWcsCircle WcsCircle { get; set; }

        public FindCircle FindCircle { get => _findCircle; set => _findCircle = value; }
        ...
        public CircleMeasure()
        {
            //////////////////////
            InitBindingTable();
        }

        public CircleMeasure(string aqSourceName, IFunction coordSystem)
        {
            this._AcqSourceName = aqSourceName;
            if (this._AcqSourceName != null)
            {
        ...
        public CircleMeasure(IFunction imageSource, IFunction coordSystem)
        {
            ////////////////////////////////
            InitBindingTable();
        }
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            try
            {
                    ...
                    foreach (var item in param)
                    {
                        switch (item.GetType().Name)
                        {
                            case nameof(userWcsCircle):
                                this._findCircle.CircleWcsPosition = (userWcsCircle)item;
                                break;
                            case nameof(userPixCircle):
                                this._findCircle.CirclePixPosition = (userPixCircle)item;
                                break;
                            case nameof(userPixCoordSystem):
                ...
                if (this._pixCoordSystem == null)
                    this.Result.Succss = this._findCircle.FindCircleMethod(this.ImageData, this.PixCoordSystem, offsetCoordSys);
                else
                    this.Result.Succss = this._findCircle.FindCircleMethod(this.ImageData, this._pixCoordSystem, offsetCoordSys);
                this.WcsCircle = this._findCircle.FitCircle;
                this.WcsCircle.CamName = this._imageData.CamName;
                this.WcsCircle.ViewWindow = this._imageData.ViewWindow;
                stopwatch.Stop();
                ///////////////////////////////////////////
                this.InitBindingTable();
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[0].SetValue(this.name, "X", this._findCircle.FitCircle.X);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[1].SetValue(this.name, "Y", this._findCircle.FitCircle.Y);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[2].SetValue(this.name, "Z", this._findCircle.FitCircle.Z);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[3].SetValue(this.name, "半径", this._findCircle.FitCircle.Radius);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[4].SetValue(this.name, "直径", this._findCircle.FitCircle.Radius * 2);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[5].SetValue(this.name, "Cam_X", this._findCircle.FitCircle.X - this._imageData.Grab_X);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[6].SetValue(this.name, "Cam_Y", this._findCircle.FitCircle.Y - this._imageData.Grab_Y);
                ...
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[9].SetValue(this.name, "Grab_Z", this._imageData.Grab_Z);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[10].SetValue(this.name, "Grab_Theat", this._imageData.Grab_Theta);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[11].SetValue(this.name, "Grab_U", this._imageData.Grab_U);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[12].SetValue(this.name, "Grab_V", this._imageData.Grab_V);
                if (this.Result.Succss)
                    ((BindingList<MeasureResultInfo>)this.ResultInfo)[13].SetValue(this.name, "执行结果", 1);
                else
                    ((BindingList<MeasureResultInfo>)this.ResultInfo)[13].SetValue(this.name, "执行结果", 0);
                ((BindingList<MeasureResultInfo>)this.ResultInfo)[14].SetValue(this.name, "Time", stopwatch.ElapsedMilliseconds);
                ///////////////////////////
                LoggerHelper.Info(this.name + ":" + this._findCircle.FitCircle.ToString());
                OnExcuteCompleted(this._imageData.CamName, this._imageData?.ViewWindow, this.name + index, this._findCircle.FitCircle);
            }
            catch (Exception ex)
            {
        ...
        public object GetPropertyValues(string propertyName)
        {
            switch (propertyName)
            {
                ...
                case nameof(WcsCircle):
                default:
                    return this.WcsCircle;
            }
        }
