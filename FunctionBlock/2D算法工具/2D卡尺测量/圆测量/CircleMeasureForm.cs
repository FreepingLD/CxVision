...
{
    public partial class CircleMeasureForm : Form
    {
        ...
        public CircleMeasureForm(IFunction function, TreeNode node)
        {
            this._refNode = node;
            this._function = function;
            InitializeComponent();
            this.Text = function.GetPropertyValues("名称").ToString();
            this.drawObject = new userDrawCircleMeasure(this.hWindowControl1, ((CircleMeasure)_function).FindCircle.CirclePixPosition.AffineTransPixCircle(((CircleMeasure)_function).PixCoordSystem?.GetVariationHomMat2D()), ((CircleMeasure)_function).PixCoordSystem);
            new ListBoxWrapClass().InitListBox(this.listBox1, node);
            new ListBoxWrapClass().InitListBox(this.listBox2, node, 2);
        }
        private void CircleMeasureForm_Load(object sender, EventArgs e)
        {
            /////////////////
            this.drawObject.BackImage = ((FunctionBlock.CircleMeasure)_function).ImageData;
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.CircleMeasure)this._function), "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                switch (e.ClickedItem.Name)
                {
                    ...
                    case "执行":
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixCircleParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.CircleMeasure)this._function).WcsCircle.GetPixCircle().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((CircleMeasure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((CircleMeasure)this._refNode.Tag).FindCircle.CirclePixPosition.AffineTransPixCircle(((CircleMeasure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((CircleMeasure)this._refNode.Tag).ImageData != null ? ((CircleMeasure)this._refNode.Tag).ImageData : null;
        ...
        private void CircleMeasureForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                ((CircleMeasure)_function).FindCircle.MetrolegyComplete -= new MetrolegyCompletedEventHandler(this.MetrolegyCompleted);
                this.drawObject.GrayValueInfo -= new GrayValueInfoEventHandler(hWindowControl1_MouseMove);
                BaseFunction.ExcuteCompleted -= new ExcuteCompletedEventHandler(DisplayExcuteResult);
                if (cts != null)
                    cts.Cancel();
