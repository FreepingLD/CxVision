...
{
    public partial class Rect2MeasureForm : Form
    {
        private object _objectDataModel;
        private Form form;
        private IFunction _function;
        private userDrawRect2Measure drawObject;
        private TreeNode _refNode;
        private MetrolegyParamForm metrolegyParam;
        public Rect2MeasureForm(IFunction function,TreeNode node)
        {
            this._refNode = node;
        ...
        private void Rect2MeasureForm_Load(object sender, EventArgs e)
        {
            this.drawObject.GrayValueInfo += new GrayValueInfoEventHandler(hWindowControl1_MouseMove);
            BaseFunction.ExcuteCompleted += new ExcuteCompletedEventHandler(DisplayExcuteResult);
            ////////////////////
            this.drawObject.BackImage = ((FunctionBlock.Rectangle2Measure)_function).ImageData;
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.Rectangle2Measure)this._function), "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                ///////////////////
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixRectangle2Param(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.Rectangle2Measure)this._function).WcsRect2.GetPixRectangle2().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((Rectangle2Measure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((Rectangle2Measure)this._refNode.Tag).FindRect2.Rect2PixPosition.AffineTransPixRect2(((Rectangle2Measure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((Rectangle2Measure)this._refNode.Tag).ImageData != null ? ((Rectangle2Measure)this._refNode.Tag).ImageData : null;
