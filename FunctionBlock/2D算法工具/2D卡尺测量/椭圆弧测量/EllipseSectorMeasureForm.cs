...
{
    public partial class EllipseSectorMeasureForm : Form
    {
        private object _objectDataModel;
        private Form form;
        private IFunction _function;
        private userDrawEllipseSectorMeasure drawObject;
        private TreeNode _refNode;
        private MetrolegyParamForm metrolegyParam;
        public EllipseSectorMeasureForm(IFunction function,TreeNode node)
        {
            this._refNode = node;
        ...
        private void LineMeasureForm_Load(object sender, EventArgs e)
        {
            ...
            //////////////////////
            this.drawObject.BackImage = (((EllipseSectorMeasure)_function).ImageData);
            BindProperty();
        }
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.EllipseSectorMeasure)this._function), "AcqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                ///////////////////
            ...
            try
            {
                switch (e.ClickedItem.Name)
                {
                    ...
                    case "执行":
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixEllipseSectorParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.EllipseSectorMeasure)this._function).WcsEllipseSector.GetPixEllipseSector().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((EllipseSectorMeasure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((EllipseSectorMeasure)this._refNode.Tag).FindEllipseSector.EllipseSectorPixPosition.AffineTransPixEllipseSector(((EllipseSectorMeasure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((EllipseSectorMeasure)this._refNode.Tag).ImageData != null ? ((EllipseSectorMeasure)this._refNode.Tag).ImageData : null;
        ...
        private void FindLineForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                ((EllipseSectorMeasure)_function).FindEllipseSector.MetrolegyComplete -= new MetrolegyCompletedEventHandler(this.MetrolegyCompleted);
                this.drawObject.GrayValueInfo -= new GrayValueInfoEventHandler(hWindowControl1_MouseMove);
                BaseFunction.ExcuteCompleted -= new ExcuteCompletedEventHandler(DisplayExcuteResult);
            }
            catch
