...
{
    public partial class CircleSectorMeasureForm : Form
    {
        private object _objectDataModel;
        private Form form;
        private IFunction _function;
        private userDrawCircleSectorMeasure drawObject;
        private TreeNode _refNode;
        private MetrolegyParamForm metrolegyParam;
        public CircleSectorMeasureForm(IFunction function, TreeNode node)
        {
            this._refNode = node;
            this._function = function;
            InitializeComponent();
            this.Text = function.GetPropertyValues("名称").ToString();
            this.drawObject = new userDrawCircleSectorMeasure(this.hWindowControl1, ((CircleSectorMeasure)_function).FindCircleSector.CircleSectorPixPosition.AffineTransPixCircleSector(((CircleSectorMeasure)_function).PixCoordSystem?.GetVariationHomMat2D()), ((CircleSectorMeasure)_function).PixCoordSystem);
           
            new ListBoxWrapClass().InitListBox(this.listBox1, node);
            new ListBoxWrapClass().InitListBox(this.listBox2, node, 2);
        }
        public CircleSectorMeasureForm(IFunction function)
        {
            this._function = function;
            InitializeComponent();
            this.Text = function.GetPropertyValues("名称").ToString();
            this.drawObject = new userDrawCircleSectorMeasure(this.hWindowControl1, ((CircleSectorMeasure)_function).FindCircleSector.CircleSectorWcsPosition);
            new ListBoxWrapClass().InitListBox(this.listBox1, function);
            new ListBoxWrapClass().InitListBox(this.listBox2, function, 2);
        }
        private void CircleSectorMeasureForm_Load(object sender, EventArgs e)
        {
            this.drawObject.GrayValueInfo += new GrayValueInfoEventHandler(hWindowControl1_MouseMove);
            BaseFunction.ExcuteCompleted += new ExcuteCompletedEventHandler(DisplayExcuteResult);
            //////////////////////////////////////
            this.drawObject.BackImage = (((CircleSectorMeasure)_function).ImageData);
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.相机采集源comboBox.DataBindings.Add(nameof(this.相机采集源comboBox.SelectedItem), ((FunctionBlock.CircleSectorMeasure)this._function), "AqSourceName", true, DataSourceUpdateMode.OnPropertyChanged);
                /////////////////// 
            ...
            try
            {
                switch (e.ClickedItem.Name)
                {
                    ...
                    case "执行":
                        ...
                        Task.Run(() =>
                        {
                            if (this._function.Execute(this.drawObject.GetPixCircleSectorParam(), this._refNode).Succss)
                            {
                                this.Invoke(new Action(() =>
                                {
                                    ...
                                    this.drawObject.DetachDrawingObjectFromWindow();
                                    this.drawObject.AddViewObject(new ViewData(((FunctionBlock.CircleSectorMeasure)this._function).WcsCircleSector.GetPixCircleSector().GetXLD()));
                                }));
                            }
                            else
                            {
        ...
        public void 示教点位button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.drawObject?.SetParam(((CircleSectorMeasure)this._refNode.Tag).PixCoordSystem);
                this.drawObject?.SetParam(((CircleSectorMeasure)this._refNode.Tag).FindCircleSector.CircleSectorPixPosition.AffineTransPixCircleSector(((CircleSectorMeasure)this._refNode.Tag).PixCoordSystem?.GetVariationHomMat2D()));
                this.drawObject.BackImage = ((CircleSectorMeasure)this._refNode.Tag).ImageData != null ? ((CircleSectorMeasure)this._refNode.Tag).ImageData : null;
        ...
        private void FindLineForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                this.drawObject.GrayValueInfo -= new GrayValueInfoEventHandler(hWindowControl1_MouseMove);
                ((CircleSectorMeasure)_function).FindCircleSector.MetrolegyComplete -= new MetrolegyCompletedEventHandler(this.MetrolegyCompleted);
                BaseFunction.ExcuteCompleted -= new ExcuteCompletedEventHandler(DisplayExcuteResult);
            }
            catch
            {
