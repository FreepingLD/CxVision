...
namespace FunctionBlock
{
    ...
    public class DataCodeDetection : BaseFunction, IFunction // 实现接口的类里，只包含输入、输出函数及接口方法
    {
        [NonSerialized]
        ...
        public DoDataCodeDetection DoDataCode { get; set; }
        ...
        public DataCodeDetection()
        {
            this.DoDataCode = new DoDataCodeDetection();
        ...
        public OperateResult Execute(params object[] param)
        {
            ...
            try
            {
                ...
                stopwatch.Restart();
                DataCodeResult DataCodeResult = new DataCodeResult();
                Result.Succss = this.DoDataCode.FindDataCode2D(this.ImageData.Image, out DataCodeResult);
                stopwatch.Stop();
        ...
        public void ReleaseHandle()
        {
            try
            {
                this.DoDataCode.ClearDataCode2dModel();
        ...
        public void Read(string path)
        {
            try
            {
                string modelPath = path.Replace(".txt", ".dmc");
                this.DoDataCode?.ReadDataCodeModel(modelPath);
            }
            catch (Exception ex)
            {
                MessageBox.Show(this.name + "->读取数据码失败" + ex.ToString());
            }
        }
        public void Save(string path)
        {
            ...
            {
                string modelPath = path.Replace(".txt", ".dmc");
                this.DoDataCode?.SaveDataCodeModel(modelPath); // 保存形状模型
            }
            catch (Exception ex)
            {
                MessageBox.Show(this.name + "->保存数据码失败" + ex.ToString());
            }
            /////////////////////////////////////////////// 以xml的形式保存参数 //////////////////
        }
        #endregion
