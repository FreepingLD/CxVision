...
{
    public partial class ContToLineDist2DForm : Form
    {
        private IFunction _function;
        private VisualizeView drawObject;

        public ContToLineDist2DForm(IFunction function,TreeNode node)
        {      
        ...
        private void ContToLineDist2DForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        private void BindProperty()
        {
            try
            {
                ...
                this.输出点comboBox.DataSource = Enum.GetNames(typeof(enOutPointMode));
                this.计算数据点comboBox.DataSource = Enum.GetNames(typeof(enCalculateDataPoint));
                this.输出点comboBox.DataBindings.Add("Text", ((NPointToLineDist2D)this._function).Param, "OutPointMode", true, DataSourceUpdateMode.OnPropertyChanged);
                this.平均点comboBox.DataBindings.Add("Text", ((NPointToLineDist2D)this._function).Param, "Averange", true, DataSourceUpdateMode.OnPropertyChanged);
                this.计算数据点comboBox.DataBindings.Add("Text", ((NPointToLineDist2D)this._function).Param, "DataPointSource", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
        ...
        private void ContToLineDist2DForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
