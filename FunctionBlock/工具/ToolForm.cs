            ...
            switch (name.Trim())
            {
                ...
                case "纠偏计算节点":
                    if (this._treeViewTarget != null)
                    {
                        ...
                        switch (SystemParamManager.Instance.SysConfigParam.Language)
                        {
                            default:
                            case "zh-CN":
                                rootNode = this._treeViewTarget.AddItems(rectify, "纠偏计算", nameof(rectify.WcsCoordSystem), nameof(rectify.AddXYTheta), nameof(rectify.CurWcsVector), nameof(rectify.TeachWcsVector));
                                break;
                            case "en-US":
                                rootNode = this._treeViewTarget.AddItems(rectify, "RectifyCalculate", nameof(rectify.WcsCoordSystem), nameof(rectify.AddXYTheta), nameof(rectify.CurWcsVector), nameof(rectify.TeachWcsVector));
                                break;
                        }
                    }
                    break;
                ...
                case "轮廓提取节点":
                    ...
                    switch (SystemParamManager.Instance.SysConfigParam.Language)
                    {
                        default:
                        case "zh-CN":
                            rootNode = this._treeViewTarget.AddItems(trackExtract, "轮廓提取", nameof(trackExtract.CurTrackPoint), nameof(trackExtract.StdTrackPoint), nameof(trackExtract.WcsPolyLine));
                            break;
                        case "en-US":
                            rootNode = this._treeViewTarget.AddItems(trackExtract, "ContourExtract", nameof(trackExtract.CurTrackPoint), nameof(trackExtract.StdTrackPoint), nameof(trackExtract.WcsPolyLine));
                            break;
                    }
                    break;  //
                case "轨迹偏置":
