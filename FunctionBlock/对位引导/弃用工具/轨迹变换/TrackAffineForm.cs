...
{
    public partial class TrackAffineForm : Form
    {
        protected Form form;
        protected IFunction _function;
        private VisualizeView drawObject;
        public TrackAffineForm(IFunction function)
        {
            InitializeComponent();
        ...
        public TrackAffineForm(TreeNode node)
        {
            InitializeComponent();  
        ...
        public void TrackAffineForm_Load(object sender, EventArgs e)
        {
            // 注册事件
        ...
        protected void BindProperty()
        {
            try
            {
                ...
                this.引导对象comboBox.DataBindings.Add("Text", ((TrackAffine)this._function), "Jaw", true, DataSourceUpdateMode.OnPropertyChanged);
            }
            catch (Exception ee)
            {
            ...
            try
            {
                ...
                switch (this.显示条目comboBox.Text.Trim())
                {
                    case nameof(enShowItems.输入轨迹):
                        foreach (var item in ((TrackAffine)this._function).TrackPoint)
                        {
                            list_x.Add(item.X);
                            list_y.Add(item.Y);
                            list_z.Add(item.Z);
                        ...
                        break;
                    case nameof(enShowItems.输出轨迹):
                        foreach (var item in ((TrackAffine)this._function).AffinePoint)
                        {
                            list_x.Add(item.X);
                            list_y.Add(item.Y);
                            list_z.Add(item.Z);
